...
func TestYTSAndEZTVAPISearchProducePrivateMagnetIdentities(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.kind, func(t *testing.T) {
			...
			if err != nil || len(page.Items) != 1 || page.Items[0].TorrentID == "" || page.Items[0].Seeders == nil || page.Items[0].SizeBytes == 0 {
...
func TestEZTVSizeRejectsUnsafeNumberShapes(t *testing.T) {
	for _, size := range []string{`-1`, `"-1"`, `1.5`, `"1.5"`, `1e3`, `"9223372036854775808"`, `"not-a-number"`} {
		body := []byte(`{"torrents_count":1,"limit":100,"page":1,"torrents":[{"title":"Example Show","hash":"0123456789abcdef0123456789abcdef01234567","size_bytes":` + size + `}]}`)
		if _, err := parseEZTV(body, "Example Show", 1); err == nil {
			t.Fatalf("unsafe size accepted: %s", size)
		}
	}
}
