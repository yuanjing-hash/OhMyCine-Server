...
统一“计划任务”页是所有可配置周期工作的权威编辑入口。表单以分钟、小时、每日、每周、每月和高级模式生成标准五段 Cron，同时显示时区、未来运行预览、misfire、重叠、重试和最大运行时间；不能另外维护一套只在浏览器生效的 interval。媒体库监听的增量补偿属于生活事件/文件监听运行参数，不伪装成 Cron 任务。

用户页允许在角色模板之外给单个账号配置全局或媒体库/下载器/站点范围的 allow/deny，并明确显示 deny 优先。发现详情显示持久 Acquisition 阶段；多站搜索展示总站点、pending/running/succeeded/failed、结果数、当前站点、取消与失败站点重试。媒体库页提供显式“检查目录结构”，严格按诊断 → 预览 → 确认 token → 移动修复执行，未托管残留只报告、不自动删除。
...
| 发现 / 媒体库 | 当前 Server catalog 的继续观看、收藏、TMDB 自动合集、自建合集、最近入库、媒体库和全部作品 | 不展示 Emby/Jellyfin 等外部来源的代同步历史，不接收设备 token，不承担媒体库配置/扫库操作 |
...
| 插件 | 插件浏览、权限审阅、手动安装/启用/更新/卸载和运行状态；资源站插件连接还提供固定镜像、账号密码/Cookie 登录和声明式验证码点击 | Hub 是分发站点而非 Server 运行时；插件默认无全局凭据访问；密码只进入一次性登录请求，Cookie 加密保存，验证码图片通过短期同源资源句柄读取，浏览器不持久化任何凭据 |
...
- 任务中心、STRM 运行历史、媒体库扫描记录、下载历史、整理历史、做种管理已结束记录、追更运行记录和计划任务运行记录都提供“清除历史”。操作先预览可清除/将跳过数量，只隐藏已结束且无恢复证据的展示记录；进行中、排队、暂停、等待操作、等待重试及未收口物理操作必须保留。共享 Job 只在所有管理页都不再引用后清理展示明细；人工重试已隐藏失败任务时恢复对应记录。不删除真实媒体、受管产物、Catalog、播放历史、审计或运行日志。
...
媒体库首次建立或真正更换 Storage/相对根/provider 根后，Server 等待完整目录发布和后台识别收敛，再自动运行一次目录结构诊断；同一来源版本的普通扫描、重启、页面查看与人工识别不重复运行，之后只有管理员手动“重新诊断”才再次执行。管理端持续展示 `pending/queued/running/healthy/issues/repairing/failed`，并在排队和计算期间显示安全进度与分类计数；页面始终说明诊断只读，系统失败写作“目录结构诊断系统失败”，不能暗示文件已经处理。问题页只显示可操作项，缺少季集等信息提示和已正确条目不占问题列表。同一识别记录的未识别分集合并成一个作品项，主列表显示关联文件数和少量来源预览，完整来源再按 token 分页读取。完整问题由 Server 以 50/100/200 每页分页、筛选和真实总数读取，不能依赖最多 100 条摘要样本。

目录弹窗以“待处理 / 本次已处理”展示当前账号在当前 diagnosis job 的服务端持久工作区；刷新、关闭再打开或同账号换设备都恢复同一选择，其他账号不可见。确定性的目录/文件名不一致项（包括视频、图片和字幕等伴随文件）显示“将自动整理”，不提供逐项选择仪式；全局“开始整理（先预览全部确定项）”以及按类型预览由 Server 跨分页自动纳入。冲突可选择推荐来源、指定来源、全部保留为版本或跳过，识别失败等不确定项也只采用用户明确保存的决定；批量推荐/跳过跨全部分页保存。已处理项可以修改或撤销，人工识别项可以重新选择或恢复自动识别；主动重新检测会开启空的新工作区，但不会清除 catalog 中已经确认的人工身份。预览绑定工作区 revision，选择变化立即废弃旧预览；确认后选择显示为“已提交执行”且不提供反向撤销。落选项绝不永久删除：本地进入库内受管回收目录，115 进入网盘回收站并沿用连接的定时清空配置，不支持可恢复回收时直接拒绝。人工识别后展示 Server 复验的标题、类型、年份、TMDB 与海报状态，同时明确“文件尚未整理”，直到用户另行提交修复。

作品详情的“编辑完整元数据”使用基本信息、分类与演职员、图片三个页签。字段覆盖标题、原名、日期、简介、标语、状态、评分、时长、季集统计、类型/国家/语言/工作室、导演/编剧/演员；图片页只显示 Server 返回的受验证 TMDB 候选。身份变更仍通过独立“手动识别”流程，重新刮削与清除人工匹配保留原入口。保存成功显示明确反馈；revision 冲突要求重新加载，不能把部分季保存成新状态。

下载新建页使用多行输入，一行一个链接并展示最多 50 条限制。批量提交后清除成功行、保留失败行及对应错误，便于修正后重试。三秒轮询下载列表时不得重置表单；只有用户切换下载器或来源类型才重新计算路线，目标媒体库仍有效时必须保持选择。

### 目录问题操作分流

识别冲突提供“核对识别”和跳过；重复目录记录提供“完整扫描核对”和跳过，扫描后由用户手动重新诊断。只有真实视频或伴随文件目标冲突提供推荐保留、指定来源和全部保留为版本；批量推荐不会包含前两类问题。页面操作与提交校验复用同一策略，不能被旧的“可修复”标记覆盖。需要用户决定的数量使用完整诊断总数减可修复数，不随当前分页或类型筛选变化。升级后旧摘要没有完整明细的库先显示待检查，再由后台进行一次只读兼容恢复。
...
删除媒体库统一进入持久化移除任务：先停用该库并阻止新任务，再停止本库 supervisor、取消冻结的本库扫描/识别/诊断/产物任务，并等待已经进入外部 I/O 的调用真实退出。首次基线尚未建立、已经有快照或正在转换的库都使用同一流程；首次请求统一返回 HTTP 202 `{deleted:false,status:"deleting",job_id}`，完成后重复请求返回 HTTP 200 `{deleted:true}`。

移除会分批清除此库的配置、目录索引、受管产物数据库清单、扫描/识别/诊断/产物执行记录、任务历史、write receipts、cleanup claims 和 physical ledger，但绝不删除来源媒体或磁盘/云端上的实际 NFO/JPG/STRM 文件，也不删除账号播放历史、合集本体或其他库成员。正在执行的外部调用无法立即退出时，任务保持“正在移除”并自动等待重试，不要求用户手工清任务，也不能因一个库而取消共享 downloader/provider 任务。

前端保留“正在移除索引”的库行与任务中心入口，后台按最多 250 行独立事务处理；刷新短暂失败继续轮询。删除中不显示空目录冒充正常结果，不允许扫描、编辑或整理，并关闭其他会话发起移除时仍打开的诊断窗口。目录、配置、物理写入和新任务共用持久移除门禁；在外部 I/O 仍处于 entered 时完整保留恢复证据，退出并进入 quiescent 后才允许清除此库恢复状态。HTTP 200/202 结果和列表/详情轮询均为 `no-store`。

### 诊断冲突分类计数


已有剧库识别按作品目录聚合：除 `Season 01` 等纯季目录外，`作品.S04.2005.1080p` 等季发行目录只有在包含一个明确季标记、没有集标记、与文件季号一致、前缀作品名与直接外层作品名一致时，才归入外层作品。季首播年不替换剧首播年；不一致或歧义时不猜测合并。普通重新扫描通过既有目录发布重新分组自动识别项，不增加历史修补迁移。人工识别仍保留已确认的物理文件范围；既有待识别文件不能因暂时没有 recognition ID 而被当成新分集继承邻近人工结果。

`GET /api/v1/media-libraries/:id/structure` 的 `classifications` 分别返回 `duplicate_target`（真实文件目标冲突）、`recognition_suspect_conflict`（识别冲突）和 `catalog_duplicate_conflict`（目录事实重复）。三个计数均按当前 library、diagnosis job、generation 的问题组计算，不按来源文件数计算，分类按钮与分页 `code` 筛选一一对应。后两项为新增字段，原 `duplicate_target` 不再聚合它们。

已正确识别作品的路径差异进一步拆为 `naming_mismatch`（命名不规范）和 `location_mismatch`（目录位置不规范）。前者只允许当前 Profile 模板中由标题、年份、季、集或版本占位符控制的目录段及文件名发生变化；层级数量变化、分类/路由目录变化，或移动同时伴随改名，都归后者。管理端按精确 `code` 独立分页和计数，并可通过服务端处理工作区将当前分类的全部问题跨页标记为“本次跳过”；该选择只绑定当前诊断任务，不移动文件，也不形成永久忽略规则。旧诊断的 `path_mismatch` 仅保留读取和修复兼容，重新检测后使用新分类。

识别冲突只提供核对识别和跳过，目录事实重复只提供完整扫描和跳过；它们不能借分类按钮进入保留文件/回收文件操作。SQL schema migration 95 通过现有事务迁移框架添加两个缓存计数字段，并依据相同作用域下的持久问题重算这三个计数；不清空任务、诊断、人工选择或媒体数据，不在运行时增加历史版本修补分支。

媒体库 `readiness_status=busy` 时，状态文案统一为“正在自动核验文件结果”。详情页必须写明无需手动确认、排队中的诊断会在核验后自动继续，并提供任务中心入口。不得展示没有后端动作的“待确认”按钮或文案。清除历史时若有记录仍在执行、等待处理或自动核验，弹窗分别说明已清数量与暂时保留数量，并提示结束后可再次清除。

### 媒体库待检查事件

媒体库详情的“状态”页显示“待检查事件”，只读查询
`GET /api/v1/media-libraries/{id}/provider-events?page=1`。要求该库的
`media_libraries.read` 权限，响应 `Cache-Control: no-store`；页码为正整数，
固定每页 50 条。成功响应的数据为 `{list, total, page, page_size}`，每项只有
`name`（安全文件或目录名）、`reason`（明确的本地化暂停原因）和 `created_at`。
不返回原始事件、完整路径、来源指纹、Provider ID、凭据或地址。

列表只包括未完成且暂停自动处理的通知，不代表对应文件已经清理。用户可刷新、
前后翻页；切换媒体库时取消旧请求，读取超时或失败提供重试，不触发扫描或删除。
缺少归属、来源不一致、事件顺序无法确认及目录超过安全处理范围分别解释原因。
需要补充证据时引导用户执行媒体库“完整核对”，残留 STRM 由 STRM 管理的
“完整核对”处理；不提供凭空确认删除的按钮。

### 云端空媒体目录清理

115 媒体库的“设置”页提供“云端空媒体目录清理”，默认不勾选；新增媒体库时位于
“扫描、限速与匹配配置”折叠区。仅有媒体库修改权限的用户可以保存，其他存储类型不显示。
开关独立于 STRM/302，不要求用户额外配置本地输出目录。
文案说明只回收删除事件影响且确认为空的目录；有 NFO、海报、字幕等任意文件、读取失败或正在入库时保留。
原任务日志显示已回收/仍有文件保留/有任务写入跳过，不为每个目录另建任务。
取消任务后的未确认回收记录保留为放弃确认，不把它展示成已成功删除。

### 插件浏览器验证的画面与页面操作

浏览器验证区区分“更新画面”（只请求 snapshot）和“重新加载网页”（在已有会话 reload，再取 snapshot）。
重载不新建会话、不发送密码、不自动调用登录确认；后台只接受当前用户的有效会话身份，不接受任意网址。
原页面来自 POST 时提示避免重复提交，仍保留验证画面和会话。关闭或切换连接会取消在途请求，旧 reload 完成后不能再读取新连接画面。
公开第三方页面资源失败只显示固定原因与计数，不显示 URL、响应或凭据；明确这是资源网络问题而非账号过期。

### 当前诊断处理工作区

进入、刷新、重新挂载或轮询媒体库页面都不自动打开诊断窗口，也不提交检测。
用户通过“查看诊断与处理入口”只读查看，通过“检查目录结构 / 重新检查”明确发起检测。
仅为当前选中的媒体库读取一页一条问题获取摘要，避免对全部媒体库逐个查询。

问题分页响应提供当前账号、当前诊断版本的全局待处理 / 已处理总数、待整理数及两组分类计数。
分类筛选只改变列表与分页 total，不改变全局页签总数；切到“本次已处理”后分类计数随之切换。
原始检测发现数只作为历史说明，不参与当前告警或处理数量。
跳过等选择保存在本次已处理，可撤销；重新进入仍展示服务端已保存结果，不靠重新检测或清除选择修复统计。

缺失、无效或失败的摘要数量显示“—”，说明正在读取或读取失败并提供重试，不显示为健康或零问题，也不暗示需要用户确认。
跨库、关闭窗口及保存选择时取消旧读取；只接受当前诊断版本且不倒退的工作区版本。
保存期间不发起后台摘要，旧响应不得覆盖刚保存的待处理 / 已处理计数。

### 公网传输节点

下载管理下保留统一的下载、整理和做种任务视图，并增加“传输节点”管理入口。
添加下载器时选择主 Server 或某个能力完整的在线 Node；任务确认区始终展示真实
执行位置，并说明任务创建后不会换节点或回退主机。Node 只安装 OhMyCine Node，
第三方 qBittorrent 仍由管理员独立安装和配置。

创建/重新生成 enrollment 后，正式构建显示一次完整 bash/PowerShell 安装命令；
开发构建只说明缺少官方信任根。命令和一次性 token 不进入浏览器持久存储。
Node-bound 115 输入如果包含普通 HTTP(S) 离线链接，页面立即提示首版只支持磁力
和 115 分享并禁用提交；后端仍进行最终校验。
...
添加站点提供「网盘分享站」入口。下一步按「站点分类 → PanSou 服务地址 → TG 频道」排列，当前分类为「盘搜（PanSou）」。新建预填 `https://so.252035.xyz`，用户可修改；编辑已有站点保留保存的地址。显示名称、网盘类型（当前 115）和可选服务认证位于后续配置区。搜索、站点卡片和订阅统一显示「网盘分享站」。保存仍经过现有候选配置校验，默认地址不会触发后台自动添加或自动搜索。
