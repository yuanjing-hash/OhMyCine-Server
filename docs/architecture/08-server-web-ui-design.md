...
媒体库首次建立或真正更换 Storage/相对根/provider 根后，Server 等待完整目录发布和后台识别收敛，再自动运行一次目录结构诊断；同一来源版本的普通扫描、重启、页面查看与人工识别不重复运行，之后只有管理员手动“重新诊断”才再次执行。管理端持续展示 `pending/queued/running/healthy/issues/repairing/failed`，并在排队和计算期间显示安全进度与分类计数；页面始终说明诊断只读，系统失败写作“目录结构诊断系统失败”，不能暗示文件已经处理。问题页只显示可操作项，缺少季集等信息提示和已正确条目不占问题列表。完整问题与冲突成员由 Server 正常化入库并以 50/100/200 每页分页、筛选和真实总数读取，不能依赖最多 100 条摘要样本。跨页选择持续保留，每项可选择整理/跳过，冲突可选择推荐来源、指定来源、全部保留为版本或跳过；“全部冲突按推荐保留/全部跳过”只改草稿并跨全部分页生效，单项选择优先，还可仅预览和提交当前问题类型。预览展示冻结的问题、移动、可恢复回收和跳过计数，只有再次确认才进入持久修复 Job；来源、诊断、规则或计划变化时 Server 拒绝旧预览。落选项绝不永久删除：本地进入库内受管回收目录，115 进入网盘回收站并沿用连接的定时清空配置，不支持可恢复回收时直接拒绝。人工识别后原问题展示已复验的标题、类型、年份、TMDB 与海报状态，同时明确“文件尚未整理”，直到用户另行提交修复。
