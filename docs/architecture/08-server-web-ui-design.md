...
基础目录发布后，Server 把目录结构诊断放进可恢复、按媒体库合并的后台任务；管理端持续展示 `pending/queued/running/healthy/issues/repairing/failed`，并在排队和计算期间显示安全进度与分类计数。查看已有诊断只读取投影，不会重复排队；只有明确点击“检查目录结构”或作品身份变化才发起新诊断。页面始终说明诊断只读，系统失败写作“目录结构诊断系统失败”，不能暗示文件已经处理。诊断页把可选整理建议、真正需处理项目和无需处理的缺少季集提示分开；入口由权威计数驱动，不依赖最多 100 条代表样本是否刚好包含某一类型。未识别项直接进入媒体清单手动整理，模板问题进入规则管理，冲突与非法路径可筛选安全路径并在来源侧处理后重新诊断。代表样本跨现存分类确定性取样，避免高频缺集或路径差异隐藏冲突；目标冲突行在当前路径下展示最多 20 条安全相对来源路径与完整来源数，保留仅大小写不同的冲突端，并明确标注被省略的剩余数量。预览失败在弹窗内显示并可重试；冲突仍不自动覆盖或删除，真正移动必须再次确认并进入独立持久任务。
