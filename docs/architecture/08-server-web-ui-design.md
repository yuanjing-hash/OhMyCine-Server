...
| 下载管理 | 顶部切换进行中、历史记录、新建下载、做种管理和下载器管理；新建下载的目标库使用 Server route preview，任务行展示冻结的同源本地、同源云端或跨数据源暂存路线；进行中按完整下载→整理→做种流水线收口，历史支持安全记录删除 | 下载器连接仍不决定最终媒体库；浏览器不得按 provider/storage 类型复制兼容矩阵；完整成功历史删除不再次操作 provider 或真实文件 |
| 媒体整理 | 顶部切换进行中/历史记录；展示下载完成后自动生成的冻结入库路线、分类规则应用结果、元数据匹配、命名/转移记录、失败重试、冲突处理和终态记录删除 | 分类规则定义与存储目标配置归对应设置；手动选择文件并整理归“文件管理”；删除记录不删除下载内容或媒体库文件 |
...
Server 更新是系统设置页中的管理员专属面板，不进入普通用户头像菜单。面板及其“检查、切换通道、下载并更新”动作都由 `system.admin` 裁剪；前端隐藏只改善体验，四个 `/api/v1/system/update*` 路由和 service 仍独立执行管理员授权、CSRF、单飞和审计。

面板必须同时显示当前版本、当前 Beta/Stable 通道、最新版本、上次检查时间和阶段。`dev`/不可比较构建、容器、只读安装、不支持的平台与显式 managed 部署都保留版本检查，但安装按钮禁用并显示“由部署方式管理”或首个版本需手工安装的说明。Stable 通道没有正式 Release 时显示真实空状态，不借用 Beta 结果。

安装接受后，页面进入明确的重连状态并容忍旧进程退出期间的网络失败；只有重新连接后的 `current_version` 等于请求目标版本且状态不再运行中，才显示更新成功。旧进程在退出前返回的状态不能被误判为恢复；`failed`/`rolled_back` 立即停止等待并显示稳定中文错误，超时则保留手工刷新/部署诊断指引。恢复后强制刷新认证 bootstrap 和更新状态。窄屏下四项版本摘要降为双列/单列，通道选择和按钮不依赖 hover，运行状态使用文字与 `aria-live`，不能只靠颜色。
