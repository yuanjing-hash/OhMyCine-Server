...
| 日志中心 | 打开可筛选的日志抽屉/独立页，包含运行与审计子页 | 运行日志需未来 `logs.read`，审计日志需现有 `audit.read`；内容继续脱敏 |
...
- 日志中心是历史查询面：运行日志用于脱敏诊断，审计日志用于不可由普通 UI 操作改写的安全/配置变更记录；两者分别受未来 `logs.read` 与现有 `audit.read` 约束。
