...
首次扫描完成且目录结构异常时，页面只弹出一次诊断提示，明确“暂不修复”和“修复全部可修复项目”。诊断表仅展示安全相对路径、期望路径、作品和可修复状态；未识别项引导到媒体清单手动识别，不允许盲目改名。媒体库卡片持续展示 `pending/issues/repairing/failed`，每个作品详情还提供单作品检查与修复。修复进入持久任务队列，页面只展示阶段、进度、数量和稳定错误码。
