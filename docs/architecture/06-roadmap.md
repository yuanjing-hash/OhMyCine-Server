...
- [x] 默认 TMDB 海报搜索：关键词按电影/剧集筛选和分页，点击后与推荐、相关、类似作品进入同一稳定身份详情页
- [x] 统一详情媒体库覆盖率：电影存在状态、剧集逐季逐集状态、跨库逻辑去重、Season 0 特别篇与 fail-closed unknown
- [x] 稳定身份多语言资源搜索：服务端复验 TMDB，受限聚合本地化名/地区别名/原名/英文名，跨别名去重并保留 opaque result token
- [x] 高级原始关键词资源搜索继续独立保留，不把浏览器标题伪装为可信 TMDB 身份
