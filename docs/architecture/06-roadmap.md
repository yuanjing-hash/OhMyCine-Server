...
- [~] 文件名解析、电影/剧集候选聚合与未识别兜底（已建立基础路径/文件名候选解析；完整聚合、持久化与修正工作台待后续）
- [ ] TMDB 搜索、详情补全、海报/背景缓存
- [~] OpenList/Alist 媒体库 / 文件夹视图切换与海报墙（已提供 `alist` 可见 MVP：媒体库/文件夹切换、未扫描 CTA、分类入口优先的库浏览、分类内电影/剧集/未识别海报墙和缺失海报兜底；扫描状态、结构判断和日志已收进扫描管理面板；TMDB 补全、真实海报缓存与详情聚合待后续）
