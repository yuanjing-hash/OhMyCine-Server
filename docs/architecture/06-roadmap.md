...
- [x] 媒体库作品可按媒体库与来源生命周期逻辑移出、分页查看和恢复；发现页作品卡片支持右键操作，观看记录删除通过同步标记跨设备传播。
...
已实现 PanSou 多频道聚合站点、可选加密认证、统一搜索和订阅分享转存。首批支持 115；其他网盘提供方保留类型扩展入口。使用前需配置 PanSou 服务及 115 下载器，详见 [配置说明](../deployment/tg-cloud-sites.md)。不包含 Telegram 实时监听。
