...
本轮 Server 可靠性整改尚未整体完成；十万条首次发布的 SQLite 写争用仍待下一阶段设计，见 [边界说明](09-server-reliability.md)。没有随本轮推送或发版。
...
- [ ] 实施 [一致性备份/恢复方案](10-upgrade-recovery-design.md)；现有程序回退不等于数据库恢复。
...
- [x] 快照媒体库配置/索引采用持久后台移除：明确区分 HTTP 202 已接收与实际完成，独立小批量清理、物理任务门禁、可恢复进度，保留来源文件和全部账号播放历史；快照生产转换仍受独立上线门禁控制。
...
- [~] Server 已有多版本命名与版本标签、继续观看资格、五分钟时钟校验、账号隔离历史图片接口和发现页收藏/合集分页管理；旧 Player 图片上传/资产合并仍需适配。
- [~] 任务最新请求/深链接、实时会话撤权、仪表盘事实分区及任务通知持久已读已实现，完整本地门禁与人工跨页面验收分开记录。
