...
- [~] 能拖拽文件到播放页视频区域播放；右下角悬浮播放按钮已补齐本地视频文件选择入口并跳转播放页加载，桌面文件选择/实际播放仍需原生运行环境复验
...
- [ ] 文件类型过滤 (视频文件)
- [~] 播放页视频区域拖拽播放已实现；右下角悬浮播放按钮可打开本地视频文件选择器并进入播放页，尚不等同于 LocalFileDataSource 浏览能力
- [ ] 文件关联 (双击打开)
...
- [~] 能通过文件选择器打开本地视频并进入播放页，播放页视频区域支持拖拽播放；本地文件 DataSource 浏览、文件关联与完整本地媒体库能力未完成
- [ ] 115/123/夸克在 UI 中有占位
