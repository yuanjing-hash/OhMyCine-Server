...
- [~] 播放中字幕搜索（已实现 Emby 搜索/Player 本机搜索显式二选一、所有 DataSource 可进入本机搜索、媒体名称/原始文件名/自定义关键词三选一、Emby 远程字幕 API、OpenSubtitles REST API Key 与 XML-RPC 账号密码二选一、射手网四段 MD5、迅雷 CID、本地文件直接哈希、远程播放流受限 Range 哈希、提供器独立容错、Rust 短期下载引用与 Tauri cache 即时加载；待真实账号、远程媒体和字幕结果实机验证）
