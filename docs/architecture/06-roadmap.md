...
- [~] OpenList/Alist 与本地文件递归只读扫描与扫描日志（已接入 SourceLibraryView 手动扫描、app 启动后台索引、数据源页首次无缓存时的当前源/root 前台索引提示；通过 DataSource `list()` 从源 root 递归只读读取；本地文件源使用逻辑 provider path `/...` 写入 source/root scoped raw scan cache，避免把本地绝对路径写入扫描缓存；图片二进制缓存待后续）
