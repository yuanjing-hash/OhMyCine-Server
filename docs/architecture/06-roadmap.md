...
- [x] 数据源列表管理 (添加/编辑/删除)
- [~] 刮削与分类设置页（已提供电影/剧集分组、TMDB 官方 genre 受控选项、默认分类实例、包含/排除条件、年份范围、兜底分类和本地结构化规则持久化；TMDB 凭据、扫描任务与实际规则执行待后续）
- [ ] 数据源排序设置 (决定动态侧栏展示顺序)
...
- [~] TMDB 搜索、详情补全、海报/背景缓存（已接入 TMDB token/key 设置、搜索/详情补全、poster/backdrop URL 与基于 TMDB metadata 的分类规则执行；SQLite 图片落盘缓存与手动匹配修正待后续）
- [~] OpenList/Alist 媒体库 / 文件夹视图切换与海报墙（已提供 `alist` 可见 MVP：媒体库/文件夹切换、未扫描 CTA、分类入口优先的库浏览、分类内电影/剧集/未识别海报墙和缺失海报兜底；扫描状态、结构判断和日志已收进扫描管理面板；分类优先使用 TMDB 规则结果，路径目录仅作为结构提示和兜底）
