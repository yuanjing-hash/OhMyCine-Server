...
目录结构诊断同样从 catalog 发布事务中解耦：媒体库首次建立或 Storage/相对根/provider 根身份真正变化时生成新的来源版本，完整目录发布与后台识别收敛后只自动诊断一次；普通全量/增量扫描、服务重启、页面读取和人工识别不会对已完成来源版本重复触发，管理员仍可随时手动重新诊断。后台分批读取当前 generation，并用固定 128 个本地 worker 计算视频与伴随文件目标。缺少季集等信息项不进入默认处理列表，非法路径、模板不可用、重复视频目标和伴随文件冲突按完整问题与冲突成员正常化入库并由 Server 分页。确定性的目录/文件名不一致项（包括视频与 JPG、字幕等伴随文件）在全局或按类型预览时由 Server 跨页自动纳入，不需要逐项建立人工选择；冲突、识别失败、模板及非法路径仍等待明确决定。管理端显示排队、处理中、检查进度、分类计数和 scan-run 关联；诊断始终只读，只有终态问题结果才能生成冻结预览，并在用户再次确认后进入独立修复 Job。数据库、队列或代际基础设施故障才显示“目录结构诊断系统失败”，且不改变已发布 catalog 的成功状态。
...
自动识别不会把 TMDB 第一条搜索结果直接当成答案：`internal/mediarecognition` 先把包名、provider-relative 文件集合、季集/光盘结构和 Profile 预处理结果解析成有理由的结构事实与查询变体；TMDB 召回同时覆盖 movie/tv、年份精确/±1/无年份，并只对初排前三名补充 alternative titles、translations 和季信息。统一排名器再按 Unicode/标点/空白/简繁等价标题、年份、类型、季集、文件集合、一致性、唯一性和弱流行度证据排序，对胜出身份执行 `GetByID` 复验。confidence 只负责排序、风险提示和 AI 分流，不再是 Download 或 Transfer 的硬闸门：未启用 AI 时，只要总分与标题相似度不低于版本化的 0.35 极低线，低分或同分也按稳定排序选中最高候选并标记 `provisional`；无候选或极低结果建立 `local_provisional`，进入可见待整理状态而不堵塞其它任务。启用 AI 后，`low_confidence` / `candidate_conflict` 只允许模型在最多五个既有候选中仲裁；极低或无候选时只允许模型生成结构化查询词，再由 Server 重搜和复验。AI 输出不能直接下载、移动、删除或锁定身份，失败时回到同一确定性兜底。
...
版本化目录启用后，修改媒体库来源会原子切换到新来源的空目录，旧文件不会被移动或删除；历史身份不会绑定到新来源的同名文件。Storage 根目录、115 账号或启用状态的修改需要同时更新关联目录的来源隔离边界，一次最多处理 4 个版本化媒体库；超过上限会返回冲突并提示先拆分存储来源，不会先保存一半。仅修改 Storage 名称不受该限制。存储检测只更新本次来源的健康字段，检测期间来源发生变化则要求重新检测，不会覆盖新配置。
...
MediaLibrary 的扫描器仍以只读索引为默认边界；目录写入必须经过独立的结构修复服务。首次来源版本或真正替换后的来源版本在完整扫描与识别收敛后自动诊断恰好一次，后续只在管理员手动点击时重新诊断。Server 使用与 Transfer 相同的 Profile、固定 `电影` / `电视剧` 根和命名模板生成诊断计划，只记录 `healthy/issues/failed` 状态并提示管理员，不自动移动已有内容。管理员可按问题类型或跨分页选择显式提交修复；如果旧作品未主动修复，新内容入库前必须先同步修复同 TMDB 作品，失败时阻止形成第二套目录。
...
本地扫描与云端扫描成功时都记录完整发布事实，避免初次自动诊断漏触发。已保存的诊断代次与实际执行计划代次分开：普通扫描后仍可对剩余问题重新预览、人工识别和分类型提交；已经发出的旧预览与任务继续严格校验当前来源、代次、规则和文件事实。诊断提交事务内再次核对这些边界，不能让过期后台结果覆盖新目录。

问题摘要、分类计数和分页列表统一由完整问题表投影；冲突按组计数，人工识别与修复后在同一事务更新。目录记录重复不等于真实文件重复，识别冲突也不直接回收：这两类只允许核对/跳过。预览及执行均验证来源路径和 Provider 身份，覆盖旧排队计划、无需移动的保留文件及伴随文件，防止同一实体同时进入保留和回收。

v72 仅对旧版“有问题摘要、无完整明细”的库标记一次兼容重建；从现有 catalog 恢复问题列表，不扫描网盘、不移动文件、不清空人工识别。已健康库和有效完整问题保持不变。启动恢复遇到正常扫描代次变化时推迟重试，不阻止 Server 启动。
