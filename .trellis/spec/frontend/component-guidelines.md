...
- Local subtitle settings expose two mutually exclusive OpenSubtitles authentication modes: API Key through the OpenSubtitles.com REST API, or username/password through the HTTPS OpenSubtitles.org XML-RPC account API. Users configure exactly one mode. Shooter and Xunlei remain separate provider switches; Shooter is local-file hash matching and Xunlei is an explicitly marked experimental opt-in because its fixed CID query endpoint is HTTP-only.
- Subtitle providers are independent failure boundaries. Missing OpenSubtitles credentials skip only OpenSubtitles and must never block enabled Shooter or Xunlei searches when the current playback URL is a real local absolute path; one provider failure must not discard another provider's successful results.
