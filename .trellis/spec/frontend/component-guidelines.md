...
- Local subtitle settings expose two mutually exclusive OpenSubtitles modes: API Key through the OpenSubtitles.com REST API, or username/password through the HTTPS OpenSubtitles.org legacy XML-RPC account API. Modern OpenSubtitles.com email accounts may return 401 on the legacy account system; in that exact case Player falls back to the official anonymous XML-RPC session, reports the compatibility state clearly, and keeps search/download usable instead of blocking settings. Shooter and Xunlei remain separate provider switches; both hash local files directly and remote media through bounded HTTP Range reads. Xunlei is still an explicitly marked experimental opt-in because its fixed CID query endpoint is HTTP-only.
- Subtitle providers are independent failure boundaries. Missing OpenSubtitles credentials skip only OpenSubtitles and must never block enabled Shooter or Xunlei searches when the Player has either a local absolute path or a current HTTP(S) playback request; one provider failure must not discard another provider's successful results.
- Provider capabilities must remain explicit: OpenSubtitles consumes title/custom keywords, while Shooter and Xunlei ignore keywords and match only the current video content hash. Saving a new OpenSubtitles credential enables that provider automatically. If OpenSubtitles is configured but disabled and hash providers return no match, the dialog must say that the entered keyword was not queried; never present that state as a generic all-provider zero-result search.
...
- Remote hash matching passes the current playback URL and required headers only through Tauri IPC to Rust. Rust probes total size with a one-byte Range and reads only the exact Shooter/Xunlei sample ranges, follows a bounded redirect count, drops source headers on cross-origin redirects, rejects HTTPS downgrade and non-Range responses, and never exposes playback URLs or headers to subtitle providers. External providers receive only content hashes, file basename, and language.
