...
- Android Player controls use a dedicated native-mobile overlay component instead of restyling the desktop control bar: compact title/tools at the top, three small primary transport actions centered over video, one low-height bottom timeline/tool dock, and bounded right-side sheets in landscape or bottom sheets in portrait. Keep only one visible subtitle entry in the top tool group. Preserve OhMyCine liquid-glass depth with translucent neutral layers, inner highlights, fine borders, and shadow because Android SurfaceView/WebView composition may not expose the video pixels to CSS `backdrop-filter`. Progress seeking must use Pointer Events with pointer capture so mouse and touch share one implementation.
- Subtitle search must be a dedicated full-screen mobile tool surface rather than the desktop dialog squeezed into a phone viewport. Keep search origin/keyword/language controls visible, make the result area independently scrollable, and always show explicit loading, result count, empty, download, and error states.
