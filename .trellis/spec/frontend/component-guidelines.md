...
- Subtitle providers are independent failure boundaries. A missing OpenSubtitles API Key skips only OpenSubtitles and must never block enabled Shooter or Xunlei searches for a local file; one provider failure must not discard another provider's successful results.
