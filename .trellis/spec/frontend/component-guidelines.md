...
- The left-side Android vertical gesture changes the current Activity window's display brightness and restores the prior window/system-managed value when playback exits. It must not write global Android brightness settings. The mpv `brightness` property remains a separate per-media picture adjustment exposed inside the Player picture/settings panel on desktop and mobile.
