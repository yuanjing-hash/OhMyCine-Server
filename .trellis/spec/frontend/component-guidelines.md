...
- Local subtitle settings expose two mutually exclusive OpenSubtitles authentication modes: API Key through the OpenSubtitles.com REST API, or username/password through the HTTPS OpenSubtitles.org XML-RPC account API. Users configure exactly one mode. Shooter and Xunlei remain separate provider switches; both hash local files directly and remote media through bounded HTTP Range reads. Xunlei is still an explicitly marked experimental opt-in because its fixed CID query endpoint is HTTP-only.
...
- Subtitle search offers three explicit keyword sources: scraped/display media title as the default, original file basename without any directory, and a user-entered custom keyword. Never send a directory path, signed URL, query string, credential, or local absolute path as the title query.
