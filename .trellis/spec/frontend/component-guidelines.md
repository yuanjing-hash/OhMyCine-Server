...
- Android libmpv defaults to `vo=gpu-next`. A playback may fall back to `vo=gpu` only after native mpv error logs explicitly show that the primary GPU output failed; do not silently change the global default based on device assumptions. Advanced Player settings should eventually expose renderer, hardware decoding, cache, synchronization, and color options without weakening this default.
