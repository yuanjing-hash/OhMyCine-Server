...
- Android Player controls use a dedicated native-mobile overlay component instead of restyling the desktop control bar: compact title/tools at the top, three primary transport actions centered over video, one slim bottom timeline/tool dock, and bounded right-side sheets in landscape or bottom sheets in portrait. Progress seeking must use Pointer Events with pointer capture so mouse and touch share one implementation.
