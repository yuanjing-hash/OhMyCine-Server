...
- Queue `sid`, `aid`, and `sub-add` through one ordered libmpv asynchronous command queue. Release the shared `MpvState` lock while awaiting `MPV_EVENT_COMMAND_REPLY`, return the real completion error or timeout to Vue, and dispatch the next track command only after the current reply. Do not pass stack-backed values to `mpv_set_property_async`, discard command replies, or treat queue acceptance as successful subtitle loading.
- Player-downloaded Windows subtitle cache paths may be canonicalized as `\\?\C:\...` for persistence checks, but strip the extended-length prefix before passing the path to libmpv. Preserve UNC paths by converting `\\?\UNC\server\share` to `\\server\share`.
