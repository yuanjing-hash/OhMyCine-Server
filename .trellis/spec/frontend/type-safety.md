...
- Queue `sid` and `aid` through libmpv's asynchronous `set` command. Do not pass stack-backed values to `mpv_set_property_async`; compile success is not proof that the property payload remains valid after the FFI call returns.
