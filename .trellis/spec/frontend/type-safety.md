...
#### 2. Signatures
- Vue continues to call the existing `mpv_load`, `mpv_add_subtitle`, `mpv_pause`, `mpv_resume`, `mpv_stop`, `mpv_seek`, `mpv_get_property`, `mpv_set_property`, `mpv_track_state`, render-state commands, and the typed `mpv_display_brightness_state` / `mpv_set_display_brightness` commands.
...
- Android display brightness is a bounded `0..100` Activity-window override. Read the current system level only as an initial value, preserve `BRIGHTNESS_OVERRIDE_NONE` when present, and restore the exact prior window value on stop/destroy. Global system brightness writes and `WRITE_SETTINGS` permission are forbidden.
- Windows display brightness uses the Player window's `HWND` to select its nearest logical monitor, then tries DXVA2 DDC/CI physical-monitor brightness before the `ROOT\\WMI` built-in-panel provider. Physical monitor handles must always be destroyed, range conversion must honor each monitor's minimum/maximum values, and synchronous DDC/WMI work must run through a blocking task rather than the async command executor. Unsupported monitors return `supported=false`; the frontend must not reinterpret that as mpv video brightness.
- Android background playback uses a non-sticky foreground `mediaPlayback` service plus `MediaSessionCompat`. The service may run only while libmpv has pending/loading/playing media, must stop itself for idle/ended/error playback, and must be stopped explicitly by the native `stop`/destroy path. Notification metadata and controls contain safe display title/progress only, never media URLs or provider headers.
...
- TMDB settings may override the API and artwork base URLs independently with user-controlled HTTPS proxy prefixes. Reject credentials, query strings, fragments, and non-HTTPS schemes. Each route is persisted only after its own real request succeeds; failure keeps the previously verified value active without changing the other route. Custom API routes never fall back to another domain, and native Tauri requests disable redirects so credentials cannot be forwarded silently.
- Tauri desktop and Android TMDB API traffic use the native Rust request command so connection testing and actual scraping have the same CORS-independent transport. Browser/Vite development may use `fetch` as a fallback. Cached TMDB metadata rebuilds artwork URLs from stored TMDB file paths and the current verified image base URL instead of pinning an obsolete host forever.
...
- `SubtitleTrackOption.source` distinguishes `downloaded`, `provider`, `embedded`, and metadata-only `detail` tracks. Do not infer ownership from the `external:*` selection ID prefix, which is only an internal identifier namespace.
...
- Navigation bindings cover home, settings, data-source management, and dynamic `source:<id>` targets. Player bindings take priority on the Player route; navigation bindings remain active elsewhere. Duplicate bindings within one context and bare `Space`/arrow/`Escape` are rejected.
