...
- Download cancellation is destructive by explicit product contract: after UI confirmation the worker calls provider `Cancel(..., true)`. A cancellation acknowledgement hook may transactionally delete the domain DownloadTask and its Job (thereby cascading queue attempts/events/actions); provider failure must leave those facts recoverable.
