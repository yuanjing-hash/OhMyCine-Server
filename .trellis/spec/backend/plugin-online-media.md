...
- Physical library/category DTO: `artwork_url`, `artwork_revision`, `artwork_source`.
- Online plugin library/navigation DTO: `artworkUrl`, `artworkRevision`, `artworkSource`.
...
- Plugin request may include a bounded `scopeKey` (`route:<routeKey>` or `branch:<nodeKey>`); plugin response is `[{"id":"stable-media-id","assetRef":"uuid"}]`, at most 9 rows.
- Generated asset route: `GET /api/v1/assets/generated-library-covers/:contentSha256?exp=<unix>&sig=<base64url-hmac>`.

### 3. Contracts

- Top-level Server physical and plugin libraries are stable entry cards: physical sources use their built-in local/cloud cover and plugins use Manifest `libraryArtwork`. Dynamic generation starts only after entering a physical category or plugin navigation node.
- The index owner generates the category card: Server generates for Server-managed physical/115 categories and plugin navigation nodes; Player generates for independently indexed local/raw categories. A Server DTO never exposes candidate URLs or `assetRef` values.
- Physical candidates come from distinct recognition rows that still own at least one current entry in the exact `library ID + category name + media kind` scope and contain a valid bounded TMDB poster/backdrop path. Candidate eligibility follows the category entries visible to Player, not recognition `matched` status alone: a low-confidence result may remain `unrecognized` for workflow purposes while still carrying a verified TMDB snapshot used for classification and safe artwork. Credential/network/no-match records without a valid snapshot contribute no candidate. Episode joins must not duplicate one recognition before the query limit and crowd out other works.
...
- Candidate identity is deduplicated and sorted deterministically. At most 9 distinct candidates participate in the generation key. Before rendering, 1-8 successfully decoded images are repeated deterministically until all 9 style slots are filled, matching the reference plugin's `prepare_library_images()` contract without introducing random output. The 1920x1080 JPEG uses 3x3 posters, `315426987` visibility order, 410x610 cells, 22px gaps, 46px corners, shadow, -15.8 degree column rotation, and a poster-derived left-dark/right-light gradient.
...
| One to eight candidates decode successfully | Repeat the available images into all nine slots before style-3 placement; never position a sparse column with the full three-row height |
...
- Good: a 115 entry card stays fixed while its `外语电影` category uses only that category's current entries with valid TMDB snapshots to produce a deterministic style-3 cover, including low-confidence entries already shown in that category.
- Good: Bilibili registers different Host assets for `route:recommended` and `route:anime-jp`; Server renders each navigation node and Player sees only signed same-origin JPEG URLs.
...
- Generator tests assert stable ordering, 1920x1080 output, style-3 placement, content-digest equality, bounded LRU cleanup, signed-ticket success/tamper/expiry and hostile decoded-dimension rejection.
- Database tests prove candidate isolation by library/category/media kind, low-confidence snapshot eligibility, and that many episode rows cannot crowd out distinct works in the same scope. Generator tests cover 1, 2, 3, 4, 8 and 9 decoded candidates and assert complete foreground coverage.
