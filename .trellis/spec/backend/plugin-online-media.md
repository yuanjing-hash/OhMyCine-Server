...
- Physical candidates come from distinct matched recognition rows that still own at least one current entry in the exact `library ID + category name + media kind` scope. Episode joins must not duplicate one recognition before the query limit and crowd out other works.
...
- Candidate identity is deduplicated and sorted deterministically. At most 9 candidates participate in the generation key and all 9 may be rendered into one 1920x1080 JPEG using the style-static-3 contract: 3x3 posters, `315426987` visibility order, 410x610 cells, 22px gaps, 46px corners, shadow, -15.8 degree column rotation, and a poster-derived left-dark/right-light gradient.
...
- Good: a 115 entry card stays fixed while its `外语电影` category uses only that category's matched works to produce a deterministic style-3 cover.
...
- Database tests prove candidate isolation by library/category/media kind and that many episode rows cannot crowd out distinct works in the same scope.
