...
- Player playback for a 115 catalog entry is authorized by the committed `media_library_entries` fact plus current user/library permission, enabled library/Storage, enabled Connection, provider file identity and provider-root ancestry. It never depends on `strm_enabled`, `signed_proxy_enabled`, or a `media_artifacts` row. Catalog detail exposes only a stable `server:entry:<id>` identity and authenticated stream path; the stream request uses the interactive 115 read lane to re-stat the file and revalidate root ancestry before resolving a temporary direct URL through the existing per-client playback coordinator. The already validated item is reused instead of issuing a duplicate provider `Stat`; the private coordinator/cache identity binds a one-way provider-item digest so a replaced entry cannot reuse an old URL or device lease. Provider IDs, pickcodes and temporary URLs remain private.
...
- STRM administration reads only safe library/run/artifact DTOs. An incremental/full request first schedules artifact generation for the already committed `baseline_generation` when it is newer than the applied artifact generation, then enqueues the durable `strm_reconcile` consistency scan. This makes the explicit generate action effective without waiting for another scan while still converging provider changes that have not reached the catalog. Failed artifact runs may be retried. Manual cleanup first issues an operation/actor/library/generation/root-hash/manifest-bound short-lived HMAC confirmation token. Tokens use bounded canonical Base64URL plus strict JSON and never encode the absolute projection root.
...
- The Player endpoint authenticates every bounded poll with device Bearer, bounds the decimal cursor before uint64 parsing, captures the broadcast wake generation before its first query, and revalidates the device plus current library/storage visibility after every wake or timeout. It returns only logical library ID, `content_revision`, controlled `kind`, time, cursor, and `resync_required`; revoked/disabled users, revoked devices, and newly hidden libraries cannot receive stale in-flight results. It never returns paths, provider/upstream IDs, credentials, signed URLs, or raw upstream bodies.
