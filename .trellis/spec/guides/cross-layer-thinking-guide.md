...
- [ ] When persisted artifact bytes contain an expiring signature, nonce, timestamp or rotating key identifier, separated the durable content fingerprint from lease renewal semantics. An unchanged cross-generation regression must assert bytes and mtime remain stable outside the renewal window, while near-expiry/key/origin/format changes update exactly once; never regenerate time-dependent bytes before deciding whether content is unchanged.
