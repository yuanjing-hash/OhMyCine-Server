...
// PlayerPlaybackHistory is a user-owned, provider-neutral playback projection.
// Server catalog rows use a Server-derived canonical SyncKey; other providers
// retain the bounded client key. Neither form contains credentials or stream URLs.
type PlayerPlaybackHistory struct {
	...
	ItemID           string    `gorm:"size:512;not null;default:''" json:"item_id,omitempty"`
	ItemToken        string    `gorm:"size:512;not null;default:''" json:"item_token,omitempty"`
	MediaIdentity    string    `gorm:"size:2048;not null" json:"media_identity"`
	Title            string    `gorm:"size:512;not null" json:"title"`
	DisplayTitle     string    `gorm:"size:512;not null;default:''" json:"display_title,omitempty"`
	DisplaySubtitle  string    `gorm:"size:512;not null;default:''" json:"display_subtitle,omitempty"`
	SeriesTitle      string    `gorm:"size:512;not null;default:''" json:"series_title,omitempty"`
	EpisodeTitle     string    `gorm:"size:512;not null;default:''" json:"episode_title,omitempty"`
	SeasonNumber     *int      `json:"season_number,omitempty"`
	EpisodeNumber    *int      `json:"episode_number,omitempty"`
	StreamIdentity   string    `gorm:"size:2048;not null;default:''" json:"stream_identity,omitempty"`
	MediaType        string    `gorm:"size:32;not null;default:''" json:"media_type,omitempty"`
	...
	ClientUpdatedAt  int64     `gorm:"not null" json:"updated_at"`
	Revision         uint64    `gorm:"not null;index" json:"revision"`
	CreatedAt        time.Time `gorm:"not null" json:"-"`
	UpdatedAt        time.Time `gorm:"not null" json:"-"`
}

func (PlayerPlaybackHistory) TableName() string { return "player_playback_history" }

type PlayerPlaybackHistoryRevision struct {
	ID        uint64    `gorm:"primaryKey"`
	UserID    uint      `gorm:"not null;index"`
	SyncKey   string    `gorm:"size:128;not null"`
...
// MediaCategoryArtwork is the durable active/pending projection for one
// Server-owned physical media-library category. Generated bytes are stored in
// the Server data directory and addressed by ContentHash.
type MediaCategoryArtwork struct {
	ID                   uint       `gorm:"primaryKey" json:"-"`
	ScopeKind            string     `gorm:"size:32;not null;uniqueIndex:idx_media_category_artwork_scope" json:"-"`
	LibraryID            uint       `gorm:"not null;uniqueIndex:idx_media_category_artwork_scope;index" json:"-"`
	CategoryKey          string     `gorm:"size:512;not null;uniqueIndex:idx_media_category_artwork_scope" json:"-"`
	CategoryName         string     `gorm:"size:512;not null" json:"-"`
	MediaType            string     `gorm:"size:16;not null" json:"-"`
	GenerationKey        string     `gorm:"size:64;not null;default:''" json:"-"`
	PendingGenerationKey string     `gorm:"size:64;not null;default:'';index" json:"-"`
	CandidateDigest      string     `gorm:"size:64;not null;default:''" json:"-"`
	TemplateVersion      string     `gorm:"size:128;not null" json:"-"`
	ContentHash          string     `gorm:"size:64;not null;default:'';index" json:"-"`
	RelativePath         string     `gorm:"size:256;not null;default:''" json:"-"`
	MIMEType             string     `gorm:"size:64;not null;default:'image/jpeg'" json:"-"`
	Revision             uint64     `gorm:"not null;default:0" json:"-"`
	Status               string     `gorm:"size:16;not null;default:'pending';index" json:"-"`
	LastErrorCode        string     `gorm:"size:128;not null;default:''" json:"-"`
...
type MediaLibraryStructureRepair struct {
	CurrentAction    string     `gorm:"size:16;not null;default:''" json:"-"`
	CurrentItem      string     `gorm:"size:2048;not null;default:''" json:"-"`
	CurrentBatchSize int        `gorm:"not null;default:0" json:"-"`
	ID               string     `gorm:"primaryKey;size:36" json:"id"`
