...
func newTestClient(t *testing.T) *testClient {
	...
	libraryStructure := services.NewMediaLibraryStructureService(db, audit, queue, connections, log)
	...
	api.SetPlayerOverviewService(services.NewPlayerOverviewService(playerHistory, playerMediaState, libraries))
...
func TestPlayerDeviceAuthenticationIsRevocableAndIsolatedFromBrowserAdmin(t *testing.T) {
	...
	historyBody := map[string]any{"cursor": 0, "changes": []map[string]any{{"sync_key": strings.Repeat("a", 64), "source_kind": "local", "source_id": "local-a", "media_identity": "entry|1|work|2", "title": "Movie", "position": 120, "duration": 1000, "completed": false, "updated_at": 2000}}}
