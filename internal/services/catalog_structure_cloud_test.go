...
func (d *structureDirectoryFailureDriver) CreateDirectory(ctx context.Context, parent, name string) (cloudpkg.Item, error) {
	if name == d.failName {
		if d.failure != nil {
			return cloudpkg.Item{}, d.failure
		}
...
func TestCatalogStructureCloudDirectoryReceiptAndExactAssetParent(t *testing.T) {
	for _, authExpired := range []bool{false, true} {
		t.Run(fmt.Sprintf("auth_%t", authExpired), func(t *testing.T) {
			...
			if authExpired {
				driver.failure = cloudpkg.Error(cloudpkg.CodeAuthExpired, false, errors.New("expired"))
			}
			...
			if result.ErrorCode == "" || (!authExpired && !strings.Contains(result.ErrorMessage, "媒体文件尚未移动")) || driver.moveCalls != 0 {
				t.Fatalf("partial mkdir=%+v moves=%d", result, driver.moveCalls)
			}
			if authExpired {
				var held models.MediaLibraryStructureRepair
				if err := s.db.First(&held, "id=?", repair.ID).Error; err != nil || held.Phase != "queued" || result.RetryAt == nil || result.ErrorCode != cloudpkg.CodeAuthExpired {
					t.Fatalf("auth hold=%+v result=%+v err=%v", held, result, err)
				}
			}
