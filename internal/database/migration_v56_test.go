...
func TestMigrationV56AddsRouteSnapshotsAndFreezesOneDefaultIngestLibrary(t *testing.T) {
	...
	if err := db.Omit("CloudEmptyCleanupEnabled", "DefaultIngestConnectionID", "StructureStatus", "StructureIssueCount", "StructureErrorCode", "StructureCheckedAt").Create(&libraryA).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Omit("CloudEmptyCleanupEnabled", "DefaultIngestConnectionID", "StructureStatus", "StructureIssueCount", "StructureErrorCode", "StructureCheckedAt").Create(&libraryB).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Omit("CloudEmptyCleanupEnabled", "DefaultIngestConnectionID", "StructureStatus", "StructureIssueCount", "StructureErrorCode", "StructureCheckedAt").Create(&libraryBConnection).Error; err != nil {
		t.Fatal(err)
	}
	ownerID := owner.ID
