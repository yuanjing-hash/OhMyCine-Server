...
func TestMigrateUpgradesAuthFoundationDatabaseToStorageFoundation(t *testing.T) {
	...
	if len(versions) != len(schemaMigrations()) {
		t.Fatalf("migration versions=%v, want %d migrations", versions, len(schemaMigrations()))
	}
	for index, version := range versions {
		if version != index+1 {
			t.Fatalf("nonsequential migrations=%v", versions)
		}
	}
	...
	if !db.Migrator().HasTable(&models.PlayerMediaFavorite{}) || !db.Migrator().HasTable(&models.PlayerMediaCollection{}) || !db.Migrator().HasTable(&models.PlayerMediaCollectionItem{}) {
		t.Fatal("player favorites and collections tables missing after upgrade")
	}
	...
	for _, column := range []string{"token_hash", "user_id", "device_id_hash", "device_name", "client_kind", "last_seen_at", "idle_expires_at", "absolute_expires_at", "revoked_at"} {
		if !db.Migrator().HasColumn(&models.DeviceToken{}, column) {
			t.Fatalf("Player device token column %s missing after upgrade", column)
		}
	}
	for _, index := range []string{"idx_device_tokens_token_hash", "idx_device_tokens_user_id", "idx_device_tokens_device_id_hash", "idx_device_tokens_client_kind", "idx_device_tokens_idle_expires_at", "idx_device_tokens_absolute_expires_at", "idx_device_tokens_revoked_at"} {
		if !db.Migrator().HasIndex("device_tokens", index) {
			t.Fatalf("Player device token index %s missing after upgrade", index)
		}
	}
	...
	var defaultProfile models.MediaClassificationProfile
	if err := db.Where("code = ?", "default-v1").First(&defaultProfile).Error; err != nil || defaultProfile.MovieDirectoryTemplate != "电影/{category}/{title} ({year})" || defaultProfile.TVDirectoryTemplate != "电视剧/{category}/{title} ({year})/Season {season:02}" {
		t.Fatalf("default profile type roots=%q %q err=%v", defaultProfile.MovieDirectoryTemplate, defaultProfile.TVDirectoryTemplate, err)
	}
}
...
func TestPersistentQueueMigrationPoliciesRBACAndForeignKeys(t *testing.T) {
	...
	var policies int64
	if err := db.Model(&models.QueuePolicy{}).Count(&policies).Error; err != nil || policies != 18 {
		t.Fatalf("policies=%d err=%v", policies, err)
	}
	var retirementPolicy models.QueuePolicy
	if err := db.First(&retirementPolicy, "job_type = ?", "media_library_retirement").Error; err != nil || retirementPolicy.Concurrency != 1 || retirementPolicy.ResourceConcurrency != 1 || retirementPolicy.LeaseSeconds != 30 {
		t.Fatalf("retirement policy=%+v err=%v", retirementPolicy, err)
	}
