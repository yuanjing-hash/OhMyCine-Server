...
func migrateStructureDraftPreviewRows(db *gorm.DB) error {
	statements := []string{
		`CREATE TABLE media_library_structure_repair_draft_preview_items (id INTEGER PRIMARY KEY AUTOINCREMENT, draft_id TEXT NOT NULL, ordinal INTEGER NOT NULL, action TEXT NOT NULL, kind TEXT NOT NULL, current_path TEXT NOT NULL, expected_path TEXT NOT NULL, created_at DATETIME NOT NULL, UNIQUE(draft_id, ordinal), FOREIGN KEY(draft_id) REFERENCES media_library_structure_repair_drafts(id) ON DELETE CASCADE)`,
		`CREATE INDEX idx_structure_draft_preview_items_draft ON media_library_structure_repair_draft_preview_items(draft_id, ordinal)`,
	}
...
func migrateStructureRepairCheckpoints(db *gorm.DB) error {
	statements := []string{
		`ALTER TABLE media_library_structure_repairs ADD COLUMN succeeded_items INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE media_library_structure_repairs ADD COLUMN failed_items INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE media_library_structure_repairs ADD COLUMN blocked_items INTEGER NOT NULL DEFAULT 0`,
		`CREATE TABLE media_library_structure_repair_items (id INTEGER PRIMARY KEY AUTOINCREMENT, repair_id TEXT NOT NULL, ordinal INTEGER NOT NULL, action TEXT NOT NULL, kind TEXT NOT NULL, source_relative TEXT NOT NULL DEFAULT '', target_relative TEXT NOT NULL DEFAULT '', dependency_ordinal INTEGER, status TEXT NOT NULL DEFAULT 'pending', attempt_count INTEGER NOT NULL DEFAULT 0, error_code TEXT NOT NULL DEFAULT '', error_message TEXT NOT NULL DEFAULT '', started_at DATETIME, finished_at DATETIME, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, UNIQUE(repair_id, ordinal), FOREIGN KEY(repair_id) REFERENCES media_library_structure_repairs(id) ON DELETE CASCADE)`,
		`CREATE INDEX idx_structure_repair_items_repair_status ON media_library_structure_repair_items(repair_id, status, ordinal)`,
	}
	for _, statement := range statements {
		if err := db.Exec(statement).Error; err != nil {
			return err
