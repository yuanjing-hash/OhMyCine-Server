...
$old = @{}
try {
    foreach ($name in @('OMC_ENV','OMC_SERVER_HOST','OMC_SERVER_PORT','OMC_PUBLIC_ORIGIN','OMC_DATABASE_PATH','OMC_CREDENTIAL_KEY_FILE','OMC_CREDENTIAL_MASTER_KEY','OHMYCINE_TMDB_READ_ACCESS_TOKEN','OHMYCINE_TMDB_API_KEY','OMC_TMDB_READ_ACCESS_TOKEN','OMC_TMDB_API_KEY')) { $old[$name] = [Environment]::GetEnvironmentVariable($name, 'Process') }
