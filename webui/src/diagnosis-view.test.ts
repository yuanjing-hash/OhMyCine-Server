...
describe('diagnosis dialog', () => {
  ...
  it('shows preview failures beside the preview action with safe retry guidance', async () => {
    ...
    const wrapper = await open()
    await button(wrapper, '预览').trigger('click'); await flushPromises()
	const action = button(wrapper, '预览').element.closest('.semantic-success')
  ...
  it('cannot confirm when frozen preview files are unavailable or stale', async () => {
    ...
    expect(button(wrapper, '开始整理').attributes('disabled')).toBeDefined()
  })
  ...
  it('shows paged failed repair items and retries only the existing job', async () => {
    const base = mocks.api.getMockImplementation()!
    mocks.api.mockImplementation((path, options, ...args) => {
      if (path.includes('/structure/repairs?')) return Promise.resolve({ list: [{ id: 'repair-1', job_id: 'job-1', phase: 'partial_failed', succeeded_items: 8, failed_items: 2, blocked_items: 1, total_items: 11, processed_items: 11 }], total: 1 })
      if (path.includes('/structure/repairs/repair-1/items?')) return Promise.resolve({ list: [{ ordinal: 2, action: 'move', kind: 'video', source_path: '剧/Season 02/01.mkv', target_path: '剧/Season 02/S02E01.mkv', status: 'failed', attempt_count: 1, error_code: 'file_locked', error_message: '文件正在被占用', updated_at: '' }], total: 201, page: Number(new URLSearchParams(path.split('?')[1]).get('page')), page_size: 100 })
      if (path === '/api/v1/jobs/job-1/retry') return Promise.resolve({ retried: true })
      return base(path, options, ...args)
    })
    const wrapper = await open(); await flushPromises()
    expect(wrapper.get('[aria-label="目录整理失败项"]').text()).toContain('已完成 8 项')
    expect(wrapper.get('[aria-label="目录整理失败项"]').text()).toContain('剧/Season 02/01.mkv')
    await button(wrapper, '下一页失败项').trigger('click'); await flushPromises()
    expect(mocks.api.mock.calls.some(([path]) => path.includes('items?page=2&page_size=100'))).toBe(true)
    await button(wrapper, '只重试失败项').trigger('click'); await flushPromises()
    expect(mocks.api.mock.calls.some(([path]) => path === '/api/v1/jobs/job-1/retry')).toBe(true)
  })
})
