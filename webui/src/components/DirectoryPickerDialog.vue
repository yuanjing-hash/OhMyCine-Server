...
import { directoryPickerBrowseEndpoint, directoryPickerFallbackEndpoint, directoryPickerInitialEndpoint, directoryRootLabel, FILESYSTEM_ROOTS_ENDPOINT, storageDirectoryEndpoint } from '@/directory-navigation'

const props = defineProps<{ open: boolean; storageId?: number | null; providerConnectionId?: number | null; restrictToStorage?: boolean; initialEndpoint?: string }>()
...
async function loadInitial() {
  ...
  if (props.providerConnectionId) { await load(`/api/v1/connections/${props.providerConnectionId}/directories`); return }
  if (!props.storageId) {
    const endpoint = directoryPickerInitialEndpoint(props.initialEndpoint)
    if (endpoint === FILESYSTEM_ROOTS_ENDPOINT) await load(endpoint)
    else await loadInitialWithRootsFallback(endpoint)
    return
  }
  await load(storageDirectoryEndpoint(props.storageId))
}
    ...
    <div v-if="open" class="directory-overlay fixed inset-0 z-100 flex items-center justify-center p-4" @mousedown.self="close">
      <section ref="dialog" role="dialog" aria-modal="true" aria-labelledby="directory-picker-title" class="panel flex max-h-[min(46rem,92vh)] w-full max-w-4xl flex-col overflow-hidden" @keydown="onKeydown">
        <header class="flex items-start justify-between gap-4">
          <div><h2 id="directory-picker-title" class="m-0 text-xl">{{ providerConnectionId || listing?.platform === 'pan115' ? '选择 115 网盘目录' : '选择 Server 目录' }}</h2><p class="page-description mb-0 mt-2 text-sm">{{ providerConnectionId || listing?.platform === 'pan115' ? '只显示当前 Storage 根目录及其下级文件夹。' : '这里显示运行 OhMyCine Server 的设备，不是当前浏览器所在设备。' }}</p></div>
        ...
        <div class="semantic-divider mt-5 flex flex-wrap items-center gap-2 border-y py-3" aria-label="目录面包屑">
          <button v-if="listing && !withinStorage() && !providerConnectionId" type="button" class="root-navigation" :disabled="loading || !listing.breadcrumbs?.length" @click="loadRoots">{{ directoryRootLabel(listing.platform) }}</button>
          ...
          <button v-for="crumb in withinStorage() || providerConnectionId ? [] : listing?.breadcrumbs ?? []" :key="crumb.token" type="button" class="breadcrumb-button" :disabled="loading" @click="browse(crumb.token)">{{ crumb.name }}</button>
          <span v-if="listing && !listing.breadcrumbs?.length" class="text-muted text-sm">{{ providerConnectionId ? '115 网盘根目录' : '选择一个 Server 根位置开始浏览' }}</span>
        </div>
        ...
        <div v-else class="semantic-inset mt-4 min-h-48 flex-1 overflow-y-auto p-2">
          <div v-if="listing && !listing.items?.length" class="text-subtle flex min-h-44 items-center justify-center text-sm">当前层没有可显示的子目录</div>
          ...
          <div v-if="listing?.next_page_token" class="flex justify-center p-3">
            <button type="button" class="btn-secondary" :disabled="loadingMore" @click="loadNextPage">{{ loadingMore ? '正在加载下一页…' : '加载更多目录' }}</button>
          </div>
        </div>
