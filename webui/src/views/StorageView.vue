...
const cloudCreate = ref({ name: '', cookie: '', recyclePassword: '', recycleCleanupEnabled: false, recycleCleanupCron: '0 */7 * * *', connectionID: 0, selectedPath: '', pickerToken: '', enabled: true })
const localEdit = ref({ name: '', selectedPath: '', pickerToken: '', enabled: true })
const cloudEdit = ref({ name: '', cookie: '', recyclePassword: '', clearRecyclePassword: false, recycleCleanupEnabled: false, recycleCleanupCron: '0 */7 * * *', enabled: true })
...
watch(selectedConnection, item => {
  ...
  if (item?.provider === 'pan115') cloudEdit.value = { name: item.name, cookie: '', recyclePassword: '', clearRecyclePassword: false, recycleCleanupEnabled: item.recycle_cleanup_enabled, recycleCleanupCron: item.recycle_cleanup_cron, enabled: item.enabled }
}, { immediate: true })
watch(storageConnection, item => { if (item && selectedStorage.value?.type === 'pan115') cloudEdit.value = { name: item.name, cookie: '', recyclePassword: '', clearRecyclePassword: false, recycleCleanupEnabled: item.recycle_cleanup_enabled, recycleCleanupCron: item.recycle_cleanup_cron, enabled: item.enabled } }, { immediate: true })
...
async function createPan115() {
  if (!cloudCreate.value.connectionID) {
    ...
    let confirmed = false
    if (cloudCreate.value.recycleCleanupEnabled) {
      if (!recyclePassword.trim()) return notify('启用定时清空前必须填写 115 操作密码', 'warning')
      confirmed = window.confirm('定时清空会永久删除该 115 账号回收站内的全部内容，无法恢复。确认启用？')
      if (!confirmed) return
    }
    const created = await run(async () => { const connection = await api<ConnectionSummary>('/api/v1/connections', { method: 'POST', body: JSON.stringify({ name: cloudCreate.value.name, provider: 'pan115', cookie, recycle_password: recyclePassword || undefined, recycle_cleanup_enabled: cloudCreate.value.recycleCleanupEnabled, recycle_cleanup_cron: cloudCreate.value.recycleCleanupCron, recycle_cleanup_confirmed: confirmed, enabled: true }) }); cloudCreate.value.connectionID = connection.id; notify('账号连接已保存，请选择 115 云端目录', 'success') })
    if (created) {
  ...
  await run(async () => { const item = await api<StorageSummary>('/api/v1/storages', { method: 'POST', body: JSON.stringify({ name: cloudCreate.value.name, type: 'pan115', connection_id: cloudCreate.value.connectionID, provider_picker_token: cloudCreate.value.pickerToken, enabled: cloudCreate.value.enabled }) }); selectedKey.value = `storage:${item.id}`; cloudCreate.value = { name: '', cookie: '', recyclePassword: '', recycleCleanupEnabled: false, recycleCleanupCron: '0 */7 * * *', connectionID: 0, selectedPath: '', pickerToken: '', enabled: true }; addOpen.value = false; addKind.value = null; notify('115 数据源已添加', 'success') })
}
...
async function saveCloudStorage() {
  ...
  const item = selectedStorage.value
  const connection = storageConnection.value
...
function finishConnection() {
  ...
  cloudCreate.value = { name: selectedConnection.value.name, cookie: '', recyclePassword: '', recycleCleanupEnabled: selectedConnection.value.recycle_cleanup_enabled, recycleCleanupCron: selectedConnection.value.recycle_cleanup_cron, connectionID: selectedConnection.value.id, selectedPath: '', pickerToken: '', enabled: selectedConnection.value.enabled }
  addOpen.value = true
...
async function savePan115() {
  ...
  let confirmed = false
  if (cloudEdit.value.recycleCleanupEnabled && !item.recycle_cleanup_enabled) { confirmed = window.confirm('定时清空会永久删除该 115 账号回收站内的全部内容，无法恢复。确认启用？'); if (!confirmed) return }
  await run(async () => { const payload: Record<string, unknown> = { name: cloudEdit.value.name, enabled: cloudEdit.value.enabled, revision: item.revision, recycle_cleanup_enabled: cloudEdit.value.recycleCleanupEnabled, recycle_cleanup_cron: cloudEdit.value.recycleCleanupCron, recycle_cleanup_confirmed: confirmed }; if (cloudEdit.value.cookie.trim()) payload.cookie = cloudEdit.value.cookie; if (cloudEdit.value.recyclePassword.trim()) payload.recycle_password = cloudEdit.value.recyclePassword; if (cloudEdit.value.clearRecyclePassword) payload.remove_recycle_password = true; await api(`/api/v1/connections/${item.id}`, { method: 'PATCH', body: JSON.stringify(payload) }); cloudEdit.value.cookie = ''; cloudEdit.value.recyclePassword = ''; cloudEdit.value.clearRecyclePassword = false; notify('115 数据源已保存', 'success') })
}
  ...
  <section class="mx-auto max-w-6xl">
    ...
    <section v-if="addOpen" class="panel mt-6">
      ...
      <form v-else-if="addKind === 'pan115'" class="grid gap-4 md:grid-cols-2" @submit.prevent="createPan115">
        ...
        <div v-if="!cloudCreate.connectionID" class="md:col-span-2"><label class="label">115 操作密码（安全码）</label><SecretInput v-model="cloudCreate.recyclePassword" class="input" autocomplete="new-password" maxlength="64" placeholder="用于回收站清理操作" /><p class="text-subtle mb-0 mt-2 text-xs">默认只用于精确清理 OhMyCine 临时播放副本；只有显式启用下方策略才会定时清空整个回收站。</p></div>
        <template v-if="!cloudCreate.connectionID"><label class="text-muted flex items-center gap-3 text-sm md:col-span-2"><input v-model="cloudCreate.recycleCleanupEnabled" type="checkbox" />定时自动清空整个 115 回收站</label><div v-if="cloudCreate.recycleCleanupEnabled" class="md:col-span-2 semantic-danger-text"><strong>危险：会永久删除该账号回收站中的全部内容，无法恢复。</strong><label class="label mt-3">5 段 Cron</label><input v-model="cloudCreate.recycleCleanupCron" class="input font-mono" required placeholder="0 */7 * * *" /></div></template>
    ...
    <div v-else class="mt-7 grid gap-5 xl:grid-cols-[minmax(20rem,.75fr)_minmax(30rem,1.25fr)]">
      ...
      <form v-else-if="selectedStorage" class="panel" @submit.prevent="saveCloudStorage"><div class="flex items-start justify-between gap-3"><div><h2 class="m-0">{{ selectedStorage.name }}</h2><p class="text-subtle mb-0 mt-1 text-xs">115 网盘 · {{ storageConnection?.name || '账号不可用' }}</p></div><span :class="statusClass(selectedKey)">{{ status(selectedKey) }}</span></div><div class="mt-5 grid gap-4 md:grid-cols-2"><div><label class="label">名称</label><input v-model="localEdit.name" class="input" :disabled="!canEdit" /></div><div><label class="label">账号连接</label><input class="input" :value="storageConnection?.name || '未知'" disabled /></div><div class="md:col-span-2"><label class="label">115 云端根目录</label><div class="flex gap-2"><input class="input" :value="localEdit.selectedPath" readonly /><button v-if="canBrowsePan115ForUpdate && selectedStorage.connection_id" type="button" class="btn-secondary" @click="openPicker('cloud-edit')">更换目录</button></div></div><div class="md:col-span-2"><label class="label">更换账号 Cookie（可选）</label><SecretInput v-model="cloudEdit.cookie" class="input min-h-20 font-mono text-xs" multiline :configured="Boolean(storageConnection?.credential_configured)" :load-secret="auth.can(Permissions.ConnectionsSecretsExport) && storageConnection ? credentialLoader({ resourceType: 'connection', resourceID: storageConnection.id, field: 'credential' }) : undefined" :reset-key="storageConnection?.id" :disabled="!canEdit" placeholder="留空表示继续使用已保存的 Cookie" /><p class="text-subtle mb-0 mt-2 text-xs">该账号下的其它数据源也会使用更新后的 Cookie。</p></div><div class="md:col-span-2"><label class="label">115 操作密码（安全码）</label><SecretInput v-model="cloudEdit.recyclePassword" class="input" :configured="Boolean(storageConnection?.recycle_password_configured)" :load-secret="auth.can(Permissions.ConnectionsSecretsExport) && storageConnection ? credentialLoader({ resourceType: 'connection', resourceID: storageConnection.id, field: 'recycle_password' }) : undefined" :reset-key="storageConnection?.id" maxlength="64" :disabled="!canEdit || cloudEdit.clearRecyclePassword" /><label v-if="storageConnection?.recycle_password_configured" class="text-muted mt-2 flex items-center gap-2 text-xs"><input v-model="cloudEdit.clearRecyclePassword" type="checkbox" :disabled="!canEdit || cloudEdit.recycleCleanupEnabled" />移除已保存的安全码</label><p class="text-subtle mb-0 mt-2 text-xs">留空会保留已保存密码；只有勾选移除才会清空。</p></div><label class="text-muted flex items-center gap-3 text-sm md:col-span-2"><input v-model="cloudEdit.recycleCleanupEnabled" type="checkbox" :disabled="!canEdit" />定时自动清空整个 115 回收站</label><div v-if="cloudEdit.recycleCleanupEnabled" class="md:col-span-2"><p class="semantic-danger-text m-0 text-sm"><strong>危险：每次执行都会永久清空整个回收站，无法恢复。</strong></p><label class="label mt-3">5 段 Cron</label><input v-model="cloudEdit.recycleCleanupCron" class="input font-mono" required :disabled="!canEdit" placeholder="0 */7 * * *" /><p class="text-subtle mb-0 mt-2 text-xs">上次：{{ checkedAt(storageConnection?.recycle_cleanup_last_run_at) }} · 状态：{{ storageConnection?.recycle_cleanup_last_status || 'idle' }} · 下次：{{ checkedAt(storageConnection?.recycle_cleanup_next_run_at) }}<span v-if="storageConnection?.recycle_cleanup_last_error_code"> · 错误：{{ storageConnection.recycle_cleanup_last_error_code }}</span></p></div><label class="text-muted flex items-center gap-3 text-sm"><input v-model="localEdit.enabled" type="checkbox" :disabled="!canEdit" />启用数据源</label></div><div class="mt-5 flex gap-3"><button v-if="canEdit" class="btn-secondary" :disabled="saving">保存</button><button v-if="canTest" type="button" class="btn-primary" :disabled="saving || !selectedStorage.enabled" @click="testSelected">测试</button><button v-if="canDelete" type="button" class="btn-danger" :disabled="saving" @click="deleteSelected">删除</button></div></form>
      <form v-else-if="selectedPan115" class="panel" @submit.prevent="savePan115"><div class="flex items-start justify-between gap-3"><div><h2 class="m-0">{{ selectedPan115.name }}</h2><p class="semantic-warning-text mb-0 mt-1 text-xs">账号已保存，但还没有选择云端根目录</p></div><span :class="statusClass(selectedKey)">{{ status(selectedKey) }}</span></div><div class="mt-5 grid gap-4 md:grid-cols-2"><div><label class="label">账号名称</label><input v-model="cloudEdit.name" class="input" :disabled="!canEdit" /></div><div><label class="label">账号</label><input class="input" :value="selectedPan115.account.name || '待测试'" disabled /></div><div class="md:col-span-2"><label class="label">更换 Cookie（可选）</label><SecretInput v-model="cloudEdit.cookie" class="input min-h-24 font-mono text-xs" multiline :configured="selectedPan115.credential_configured" :load-secret="auth.can(Permissions.ConnectionsSecretsExport) ? credentialLoader({ resourceType: 'connection', resourceID: selectedPan115.id, field: 'credential' }) : undefined" :reset-key="selectedPan115.id" :disabled="!canEdit" placeholder="留空表示继续使用已保存的 Cookie" /></div><div class="md:col-span-2"><label class="label">115 操作密码（安全码）</label><SecretInput v-model="cloudEdit.recyclePassword" class="input" :configured="selectedPan115.recycle_password_configured" :load-secret="auth.can(Permissions.ConnectionsSecretsExport) ? credentialLoader({ resourceType: 'connection', resourceID: selectedPan115.id, field: 'recycle_password' }) : undefined" :reset-key="selectedPan115.id" maxlength="64" :disabled="!canEdit || cloudEdit.clearRecyclePassword" /><label v-if="selectedPan115.recycle_password_configured" class="text-muted mt-2 flex items-center gap-2 text-xs"><input v-model="cloudEdit.clearRecyclePassword" type="checkbox" :disabled="!canEdit || cloudEdit.recycleCleanupEnabled" />移除已保存的安全码</label></div><label class="text-muted flex items-center gap-3 text-sm md:col-span-2"><input v-model="cloudEdit.recycleCleanupEnabled" type="checkbox" :disabled="!canEdit" />定时自动清空整个 115 回收站</label><div v-if="cloudEdit.recycleCleanupEnabled" class="md:col-span-2"><p class="semantic-danger-text m-0 text-sm"><strong>危险：每次执行都会永久清空整个回收站，无法恢复。</strong></p><label class="label mt-3">5 段 Cron</label><input v-model="cloudEdit.recycleCleanupCron" class="input font-mono" required :disabled="!canEdit" /><p class="text-subtle mb-0 mt-2 text-xs">上次：{{ checkedAt(selectedPan115.recycle_cleanup_last_run_at) }} · 状态：{{ selectedPan115.recycle_cleanup_last_status }} · 下次：{{ checkedAt(selectedPan115.recycle_cleanup_next_run_at) }}<span v-if="selectedPan115.recycle_cleanup_last_error_code"> · 错误：{{ selectedPan115.recycle_cleanup_last_error_code }}</span></p></div></div><div class="mt-5 flex flex-wrap gap-3"><button v-if="canBrowsePan115ForCreate" type="button" class="btn-primary" @click="finishConnection">选择目录并完成数据源</button><button v-if="canEdit" class="btn-secondary" :disabled="saving">保存账号</button><button v-if="canDelete" type="button" class="btn-danger" :disabled="saving" @click="deleteSelected">删除</button></div></form>
    </div>
