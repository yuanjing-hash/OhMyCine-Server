  ...
  <section class="mx-auto max-w-6xl">
    ...
    <section v-if="addOpen" class="panel mt-6">
      ...
      <form v-else-if="addKind === 'pan115'" class="grid gap-4 md:grid-cols-2" @submit.prevent="createPan115">
        ...
        <div v-if="!cloudCreate.connectionID" class="md:col-span-2"><label class="label">115 Cookie</label><textarea v-model="cloudCreate.cookie" class="input min-h-28 font-mono text-xs" required autocomplete="off" spellcheck="false" placeholder="UID=...; CID=...; SEID=...; KID=..."></textarea><p class="text-subtle mb-0 mt-2 text-xs">账号凭据使用 AES-GCM 加密；同一账号以后可直接复用并选择其它目录。</p></div>
        <div v-if="!cloudCreate.connectionID" class="md:col-span-2"><label class="label">115 回收站安全码（可选）</label><input v-model="cloudCreate.recyclePassword" class="input" type="password" autocomplete="new-password" maxlength="64" placeholder="用于精确清理 OhMyCine 临时播放副本" /><p class="text-subtle mb-0 mt-2 text-xs">只按 OhMyCine 记录的临时副本 ID 清理，不会默认清空整个回收站。</p></div>
    ...
    <div v-else class="mt-7 grid gap-5 xl:grid-cols-[minmax(20rem,.75fr)_minmax(30rem,1.25fr)]">
      ...
      <form v-else-if="selectedStorage" class="panel" @submit.prevent="saveCloudStorage"><div class="flex items-start justify-between gap-3"><div><h2 class="m-0">{{ selectedStorage.name }}</h2><p class="text-subtle mb-0 mt-1 text-xs">115 网盘 · {{ storageConnection?.name || '账号不可用' }}</p></div><span :class="statusClass(selectedKey)">{{ status(selectedKey) }}</span></div><div class="mt-5 grid gap-4 md:grid-cols-2"><div><label class="label">名称</label><input v-model="localEdit.name" class="input" :disabled="!canEdit" /></div><div><label class="label">账号连接</label><input class="input" :value="storageConnection?.name || '未知'" disabled /></div><div class="md:col-span-2"><label class="label">115 云端根目录</label><div class="flex gap-2"><input class="input" :value="localEdit.selectedPath" readonly /><button v-if="canBrowsePan115ForUpdate && selectedStorage.connection_id" type="button" class="btn-secondary" @click="openPicker('cloud-edit')">更换目录</button></div></div><div class="md:col-span-2"><label class="label">更换账号 Cookie（可选）</label><textarea v-model="cloudEdit.cookie" class="input min-h-20 font-mono text-xs" :disabled="!canEdit" placeholder="留空表示继续使用已保存的 Cookie"></textarea><p class="text-subtle mb-0 mt-2 text-xs">该账号下的其它数据源也会使用更新后的 Cookie。</p></div><div class="md:col-span-2"><label class="label">115 回收站安全码（可选）</label><input v-model="cloudEdit.recyclePassword" class="input" type="password" maxlength="64" :disabled="!canEdit || cloudEdit.clearRecyclePassword" :placeholder="storageConnection?.recycle_password_configured ? '已安全配置；留空保持不变' : '未配置'" /><label v-if="storageConnection?.recycle_password_configured" class="text-muted mt-2 flex items-center gap-2 text-xs"><input v-model="cloudEdit.clearRecyclePassword" type="checkbox" :disabled="!canEdit" />移除已保存的安全码</label><p class="text-subtle mb-0 mt-2 text-xs">只用于精确永久删除 OhMyCine 临时播放副本，不会清空其它回收站内容。</p></div><label class="text-muted flex items-center gap-3 text-sm"><input v-model="localEdit.enabled" type="checkbox" :disabled="!canEdit" />启用数据源</label></div><div class="mt-5 flex gap-3"><button v-if="canEdit" class="btn-secondary" :disabled="saving">保存</button><button v-if="canTest" type="button" class="btn-primary" :disabled="saving || !selectedStorage.enabled" @click="testSelected">测试</button><button v-if="canDelete" type="button" class="btn-danger" :disabled="saving" @click="deleteSelected">删除</button></div></form>
      <form v-else-if="selectedPan115" class="panel" @submit.prevent="savePan115"><div class="flex items-start justify-between gap-3"><div><h2 class="m-0">{{ selectedPan115.name }}</h2><p class="semantic-warning-text mb-0 mt-1 text-xs">账号已保存，但还没有选择云端根目录</p></div><span :class="statusClass(selectedKey)">{{ status(selectedKey) }}</span></div><div class="mt-5 grid gap-4 md:grid-cols-2"><div><label class="label">账号名称</label><input v-model="cloudEdit.name" class="input" :disabled="!canEdit" /></div><div><label class="label">账号</label><input class="input" :value="selectedPan115.account.name || '待测试'" disabled /></div><div class="md:col-span-2"><label class="label">更换 Cookie（可选）</label><textarea v-model="cloudEdit.cookie" class="input min-h-24 font-mono text-xs" :disabled="!canEdit" placeholder="留空表示继续使用已保存的 Cookie"></textarea></div><div class="md:col-span-2"><label class="label">115 回收站安全码（可选）</label><input v-model="cloudEdit.recyclePassword" class="input" type="password" maxlength="64" :disabled="!canEdit || cloudEdit.clearRecyclePassword" :placeholder="selectedPan115.recycle_password_configured ? '已安全配置；留空保持不变' : '未配置'" /><label v-if="selectedPan115.recycle_password_configured" class="text-muted mt-2 flex items-center gap-2 text-xs"><input v-model="cloudEdit.clearRecyclePassword" type="checkbox" :disabled="!canEdit" />移除已保存的安全码</label></div></div><div class="mt-5 flex flex-wrap gap-3"><button v-if="canBrowsePan115ForCreate" type="button" class="btn-primary" @click="finishConnection">选择目录并完成数据源</button><button v-if="canEdit" class="btn-secondary" :disabled="saving">保存账号</button><button v-if="canDelete" type="button" class="btn-danger" :disabled="saving" @click="deleteSelected">删除</button></div></form>
    </div>
