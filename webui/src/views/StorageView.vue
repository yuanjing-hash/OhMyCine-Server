...
<template>
  <section class="mx-auto max-w-6xl">
    <div class="flex flex-wrap items-end justify-between gap-4">
      <div><h1 class="m-0 text-3xl font-800">连接与存储</h1><p class="page-description mt-2 max-w-3xl">Storage 注册 Server 可安全访问的提供方根；目录选择器浏览的是 Server 设备。</p></div>
    ...
    <form v-if="createOpen" class="panel mt-6 grid gap-4 md:grid-cols-2" @submit.prevent="createStorage">
      ...
      <div class="md:col-span-2"><label class="label">Server 根目录</label><div class="flex gap-2"><input class="input min-w-0 font-mono" :value="createForm.selectedPath" readonly required placeholder="尚未选择 Server 目录" /><button v-if="auth.can(Permissions.StoragesBrowse)" type="button" class="btn-secondary shrink-0" @click="openPicker('create')">选择目录</button></div><p v-if="!auth.can(Permissions.StoragesBrowse)" class="semantic-warning-text mb-0 mt-2 text-xs">缺少 storages.browse 权限，无法浏览 Server 文件系统。</p></div>
      <label class="text-muted flex items-center gap-3 text-sm"><input v-model="createForm.enabled" type="checkbox" />创建后启用</label><button class="btn-primary md:col-span-2" :disabled="saving || !createForm.pickerToken">注册 Storage</button>
    </form>

    <section v-if="!auth.can(Permissions.StoragesRead)" class="panel mt-8 opacity-75"><h2 class="m-0 text-lg">Storage 配置不可见</h2><p class="text-subtle mb-0 mt-2 text-sm">当前权限不包含 storages.read，页面不会请求或泄露已注册根路径。</p></section>
    <div v-else-if="loading" class="text-subtle mt-8">正在读取 Storage 配置…</div>
    <div v-else-if="storages.length === 0" class="panel mt-8"><h2 class="m-0 text-lg">尚未注册 Storage</h2><p class="page-description mb-0 mt-2 text-sm">有创建和目录浏览权限的管理员可选择 Server 可见目录。Server 不会扫描媒体。</p></div>
    <div v-else class="mt-7 grid gap-5 xl:grid-cols-[minmax(20rem,.75fr)_minmax(30rem,1.25fr)]">
      <div class="panel p-2"><button v-for="storage in storages" :key="storage.id" class="semantic-list-item mb-1 w-full p-3 text-left" :class="{ 'semantic-list-item--selected': selectedID === storage.id }" @click="selectedID = storage.id"><div class="flex items-center justify-between gap-3"><strong>{{ storage.name }}</strong><span :class="probeClass(storage)">{{ probeLabel(storage) }}</span></div><div class="text-subtle mt-2 truncate font-mono text-xs">{{ storage.root_path }}</div><div class="text-muted mt-2 text-xs">可用 {{ bytes(storage.probe.free_bytes) }} / 总计 {{ bytes(storage.probe.total_bytes) }}</div></button></div>
      <section v-if="selected" class="space-y-5">
        <form class="panel" @submit.prevent="saveStorage">
          <div class="flex items-start justify-between gap-3"><div><h2 class="m-0">{{ selected.name }}</h2><p class="text-subtle mt-1 text-xs">最后探测 {{ new Date(selected.probe.last_checked_at).toLocaleString() }}</p></div><span :class="probeClass(selected)">{{ probeLabel(selected) }}</span></div>
          <div class="mt-5 grid gap-4 md:grid-cols-2"><div><label class="label">名称</label><input v-model="editForm.name" class="input" maxlength="128" :disabled="!auth.can(Permissions.StoragesUpdate)" /></div><div><label class="label">类型</label><input class="input" :value="selected.type" disabled /></div><div class="md:col-span-2"><label class="label">Server 根目录</label><div class="flex gap-2"><input class="input min-w-0 font-mono" :value="editForm.selectedPath" readonly /><button v-if="auth.can(Permissions.StoragesUpdate) && auth.can(Permissions.StoragesBrowse)" type="button" class="btn-secondary shrink-0" @click="openPicker('edit')">更换目录</button></div></div><label class="text-muted flex items-center gap-3 text-sm"><input v-model="editForm.enabled" type="checkbox" :disabled="!auth.can(Permissions.StoragesUpdate)" />启用 Storage</label></div>
        ...
        <section class="panel"><h2 class="m-0 text-lg">只读能力摘要</h2><div class="mt-4 grid gap-3 sm:grid-cols-2"><div class="semantic-inset p-3"><span class="text-subtle text-xs">目录枚举</span><strong class="mt-1 block">{{ selected.capabilities.directory_list ? '支持' : '不支持' }}</strong></div><div class="semantic-inset p-3"><span class="text-subtle text-xs">文件系统 Watch</span><strong class="mt-1 block">{{ selected.capabilities.watch ? '支持' : '当前不提供' }}</strong></div><div class="semantic-inset p-3"><span class="text-subtle text-xs">可用空间</span><strong class="mt-1 block">{{ bytes(selected.probe.free_bytes) }}</strong></div><div class="semantic-inset p-3"><span class="text-subtle text-xs">总容量</span><strong class="mt-1 block">{{ bytes(selected.probe.total_bytes) }}</strong></div></div></section>
      </section>
    </div>
