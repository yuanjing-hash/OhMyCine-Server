...
<template>
  <main class="grid min-h-screen place-items-center p-5">
    <section class="panel w-full max-w-lg p-7 sm:p-9">
      <p class="mb-2 text-xs font-700 uppercase tracking-[.25em] text-cyan-300">First launch</p>
      <h1 class="m-0 text-3xl font-800">创建实例 Owner</h1>
      <p class="mt-3 text-sm leading-6 text-slate-400">Owner 是唯一的实例所有者，首版不支持隐式转移、停用或删除。请使用至少 12 位的独立密码。</p>
      <div v-if="auth.recoveryRequired" class="mt-5 rounded-3 border border-red-400/30 bg-red-400/10 p-4 text-sm text-red-100">数据库已有用户但缺少 owner。Server 已进入安全恢复状态，不会自动创建默认管理员。请通过受控数据库恢复流程处理。</div>
