...
import TransferNodesPanel from '@/components/TransferNodesPanel.vue'
import { credentialLoader } from '@/credentials'
import { beginDownloadRetry, canCancelDownloadPipeline, downloadErrorMessage, downloadProviderStatusLabel, downloadStatusClass, downloadStatusLabel, formatBytes, formatETA, formatProgress, formatSampleTime, isDownloadHistoryTask, nodePan115UnsupportedSource, parseDownloadSourceLines, reconcileDownloadRetries, summarizeDownloaderTasks, torrentToBase64, type DownloadManagementSection, type DownloadRetryPresentations, type DownloadSourceMode } from '@/downloads'
import { previewDownloadRoutes, routeTargetByID, type DownloadRoutePreview } from '@/download-routes'
import { useAuthStore } from '@/stores/auth'
import { notify } from '@/toast'
import { retargetCompletedImport, transferRouteLabel } from '@/transfers'
import type { DownloaderSummary, DownloadSettings, DownloadTaskSummary, ListResponse, MediaLibraryDetail, SeedingTaskSummary, StorageSummary, TMDBCandidate, TransferNodeSummary } from '@/types/api'
...
const transferNodes = ref<TransferNodeSummary[]>([])
const loading = ref(true)
const saving = ref(false)
const createOpen = ref(false)
const editingID = ref<string | null>(null)
const fileInput = ref<HTMLInputElement | null>(null)
const createForm = ref({ name: '', type: 'qbittorrent' as 'qbittorrent' | 'fake' | 'pan115_offline', executionLocation: 'server' as 'server' | 'node', nodeID: '', baseURL: '', username: '', password: '', downloaderSaveRoot: '', nodeMountRoot: '', storageID: 0, directoryToken: '', directoryPath: '', autoListenLifeEvents: false, enabled: true })
const editForm = ref({ name: '', executionLocation: 'server' as 'server' | 'node', nodeID: '', baseURL: '', username: '', password: '', downloaderSaveRoot: '', nodeMountRoot: '', storageID: 0, directoryToken: '', directoryPath: '', autoListenLifeEvents: false, enabled: true, clearUsername: false, clearPassword: false })
...
const reorganizingTask = ref<DownloadTaskSummary | null>(null)
const deletingTask = ref<DownloadTaskSummary | null>(null)
const deleteSourceData = ref(false)
...
const unsupportedNodePan115Source = computed(() => selectedDownloader.value?.type === 'pan115_offline' && selectedDownloader.value.execution_location === 'node' && sourceMode.value === 'url' ? nodePan115UnsupportedSource(submitForm.value.sourceURL) : '')
const selectedRoute = computed(() => routeTargetByID(routePreview.value, submitForm.value.mediaLibraryID))
const selectedTarget = computed(() => mediaLibraries.value.find(item => item.id === selectedRoute.value?.media_library_id) ?? null)
...
function nodeSupportsDownloader(node: TransferNodeSummary, type: DownloaderSummary['type']) {
  if (node.status !== 'online') return false
  if (type === 'qbittorrent') return node.capabilities.codes.includes('qbittorrent_control') && node.capabilities.codes.includes('range_export')
  if (type === 'pan115_offline') return ['pan115_offline', 'pan115_read', 'pan115_share_receive', 'range_export'].every(code => node.capabilities.codes.includes(code))
  return false
}
const createTransferNodes = computed(() => transferNodes.value.filter(item => nodeSupportsDownloader(item, createForm.value.type)))
const editTransferNodes = computed(() => transferNodes.value.filter(item => nodeSupportsDownloader(item, editing.value?.type ?? 'fake')))
function defaultLibraryForStorage(storageID: number) { const connectionID = storages.value.find(item => item.id === storageID)?.connection_id; return connectionID == null ? null : mediaLibraries.value.find(item => item.auto_listen_default && item.connection_id === connectionID) ?? null }
const createLifeEventDefault = computed(() => defaultLibraryForStorage(createForm.value.storageID))
const editLifeEventDefault = computed(() => editing.value?.life_event_default_library_id ? { id: editing.value.life_event_default_library_id, name: editing.value.life_event_default_library_name } : defaultLibraryForStorage(editForm.value.storageID))
...
const canReadTransferNodes = computed(() => auth.can(Permissions.TransferNodesRead))
...
const canClearSeedingHistory = computed(() => auth.canAny([Permissions.JobsControlOwn, Permissions.JobsControlAll, Permissions.DownloadsManageAll]))
...
const visibleSections = computed(() => ([
  ...
  { id: 'nodes', label: '传输节点', visible: canReadTransferNodes.value },
...
function readSection(): DownloadManagementSection {
  ...
  return typeof value === 'string' && ['active', 'history', 'create', 'seeding', 'downloaders', 'nodes'].includes(value) ? value as DownloadManagementSection : 'active'
}
...
watch([() => submitForm.value.downloaderID, sourceMode], () => void loadRoutePreview())
watch(() => createForm.value.storageID, () => { if (!createLifeEventDefault.value) createForm.value.autoListenLifeEvents = false })
watch(() => editForm.value.storageID, () => { if (!editLifeEventDefault.value) editForm.value.autoListenLifeEvents = false })
...
async function load(showLoading = true, quiet = false) {
  ...
  try {
    ...
    if (canReadTransferNodes.value) requests.push(api<ListResponse<TransferNodeSummary>>('/api/v1/transfer-nodes').then(data => { transferNodes.value = data.list }))
...
async function createDownloader() {
  saving.value = true
  try {
    await api<DownloaderSummary>('/api/v1/downloaders', { method: 'POST', body: JSON.stringify({ name: createForm.value.name, type: createForm.value.type, execution_location: createForm.value.type === 'fake' ? 'server' : createForm.value.executionLocation, node_id: createForm.value.executionLocation === 'node' ? createForm.value.nodeID : undefined, base_url: createForm.value.type === 'qbittorrent' ? createForm.value.baseURL : '', username: createForm.value.type === 'qbittorrent' ? createForm.value.username : '', password: createForm.value.type === 'qbittorrent' ? createForm.value.password : '', downloader_save_root: createForm.value.type === 'qbittorrent' && createForm.value.executionLocation === 'node' ? createForm.value.downloaderSaveRoot : undefined, node_mount_root: createForm.value.type === 'qbittorrent' && createForm.value.executionLocation === 'node' ? createForm.value.nodeMountRoot : undefined, storage_id: createForm.value.type === 'pan115_offline' ? createForm.value.storageID : undefined, provider_directory_token: createForm.value.type === 'pan115_offline' ? createForm.value.directoryToken : undefined, auto_listen_life_events: createForm.value.type === 'pan115_offline' && createForm.value.executionLocation === 'server' && createForm.value.autoListenLifeEvents, enabled: createForm.value.enabled }) })
    createForm.value = { name: '', type: 'qbittorrent', executionLocation: 'server', nodeID: '', baseURL: '', username: '', password: '', downloaderSaveRoot: '', nodeMountRoot: '', storageID: 0, directoryToken: '', directoryPath: '', autoListenLifeEvents: false, enabled: true }
...
function startEdit(item: DownloaderSummary) {
  ...
  editForm.value = { name: item.name, executionLocation: item.execution_location, nodeID: item.node_id ?? '', baseURL: item.base_url, username: '', password: '', downloaderSaveRoot: item.downloader_save_root || '', nodeMountRoot: item.node_mount_root || '', storageID: item.storage_id ?? 0, directoryToken: '', directoryPath: item.provider_directory_path || '/', autoListenLifeEvents: item.auto_listen_life_events, enabled: item.enabled, clearUsername: false, clearPassword: false }
}
...
function updateDownloader(id: string) {
  const providerDirectory = editing.value?.type === 'pan115_offline' && editForm.value.directoryToken
    ...
    : {}
  return api<DownloaderSummary>(`/api/v1/downloaders/${id}`, { method: 'PATCH', body: JSON.stringify({ name: editForm.value.name, execution_location: editForm.value.executionLocation, node_id: editForm.value.executionLocation === 'node' ? editForm.value.nodeID : undefined, base_url: editForm.value.baseURL, username: editForm.value.username || undefined, password: editForm.value.password || undefined, downloader_save_root: editForm.value.executionLocation === 'node' ? editForm.value.downloaderSaveRoot : undefined, node_mount_root: editForm.value.executionLocation === 'node' ? editForm.value.nodeMountRoot : undefined, clear_username: editForm.value.clearUsername, clear_password: editForm.value.clearPassword, auto_listen_life_events: editing.value?.type === 'pan115_offline' && editForm.value.autoListenLifeEvents, enabled: editForm.value.enabled, ...providerDirectory }) })
}
...
async function submitDownload() {
  ...
  if (unsupportedNodePan115Source.value) { notify('传输节点首版只支持 115 磁力离线和 115 分享，普通 HTTP(S) 离线链接请改用主 Server 下载器', 'warning'); return }
...
function canReorganize(task: DownloadTaskSummary) { return canControl(task) && task.transfer_task_id !== '' && task.transfer_phase === 'completed' && task.transfer_job_status === 'completed' && task.identity_revision > 0 }
function openReorganization(task: DownloadTaskSummary) { reorganizingTask.value = task }
async function reorganizationQueued() { await load(false, true) }
...
function isCompletedRecognitionFailure(task: DownloadTaskSummary) {
  return task.job_status === 'failed' && (task.scrape_status === 'completed_unrecognized' || task.last_error_code === 'transfer_episode_unrecognized')
}

async function cancelDownloadPipeline(task: DownloadTaskSummary) {
  if (!window.confirm(`确认取消“${task.display_name}”的后续处理？\n\nServer 会从下载器删除任务但保留已下载文件，并停止 OhMyCine 的识别、整理、入库和重试。`)) return
  saving.value = true
  try {
    await api(`/api/v1/downloads/${task.id}/cancel`, { method: 'POST', body: '{}' })
    notify('下载流水线已取消，下载器任务已移除，已下载文件保留', 'success')
    await selectSection('history')
  } catch (reason) { notify(message(reason), 'error') } finally { saving.value = false; await load(false, true) }
}

function downloadIdentityLabel(task: DownloadTaskSummary) {
  if (task.identity_locked || task.identity_source === 'manual') return `人工锁定 · r${task.identity_revision}`
  if (task.identity_source === 'ai') return `AI 辅助 · r${task.identity_revision}`
  if (task.identity_status === 'local_provisional') return `本地暂定 · r${task.identity_revision}`
  if (task.identity_status === 'provisional') return `自动暂定 · r${task.identity_revision}`
  if (task.identity_source === 'direct_id') return `TMDB ID 验证 · r${task.identity_revision}`
  return task.identity_revision > 0 ? `自动识别 · r${task.identity_revision}` : ''
}
...
function deleteDownload(task: DownloadTaskSummary) {
  deletingTask.value = task
  deleteSourceData.value = false
}
...
async function confirmDeleteDownload() {
  ...
  saving.value = true
  try {
    await api(`/api/v1/downloads/${task.id}?delete_data=${deleteSourceData.value}`, { method: 'DELETE', body: '{}' })
    tasks.value = tasks.value.filter(item => item.id !== task.id)
    notify(deleteSourceData.value ? '下载器任务、源文件和 OhMyCine 记录已删除' : '下载器任务和 OhMyCine 记录已删除，源文件已保留', 'success')
...
function downloaderLocation(item: DownloaderSummary) { return item.execution_location === 'node' ? `传输节点 · ${item.node_name || '节点不可用'}` : '主 Server' }
function downloaderOption(item: DownloaderSummary) { return `${item.name} · ${item.type === 'pan115_offline' ? '115 离线' : item.type} · ${downloaderLocation(item)}` }
...
function transferPhaseLabel(phase: string) {
  return ({ queued: '等待入库', routing: '确认入库路线', reserving_space: '检查暂存空间', materializing: '拉取到 Server 暂存', verifying: '校验完整性', recognizing: '识别与刮削', planning: '规划目录', checking_directories: '检查目标目录', creating_directories: '创建目标目录', checking_conflicts: '检查文件冲突', moving: '移动 / 复制文件', renaming: '重命名文件', uploading: '上传到目标网盘', placing: '写入目标媒体库', risk_backoff: '115 风控退避', transferring: '正在入库', reconciling: '媒体库对账', generating_artifacts: '生成 STRM / 元数据', notifying: '刷新媒体服务器', completed: '入库完成', failed: '入库失败' } as Record<string, string>)[phase] ?? '下载完成后入库'
}

let refreshTimer: number | undefined
onMounted(async () => { await load(); refreshTimer = window.setInterval(() => { void load(false, true) }, 3000) })
onBeforeUnmount(() => { if (refreshTimer !== undefined) window.clearInterval(refreshTimer); routePreviewRequest?.abort() })
</script>
...
<template>
  <section class="mx-auto max-w-7xl">
    ...
    <form v-if="activeSection === 'create' && auth.can(Permissions.DownloadsCreate)" id="download-panel-create" class="panel mt-6" role="tabpanel" aria-labelledby="download-tab-create" @submit.prevent="submitDownload">
      ...
      <div class="mt-5 grid gap-4 md:grid-cols-2"><div><label class="label">下载器</label><select v-model="submitForm.downloaderID" class="input" required><option value="" disabled>请选择</option><option v-for="item in enabledDownloaders" :key="item.id" :value="item.id">{{ downloaderOption(item) }}</option></select><p v-if="selectedDownloader" class="text-subtle mb-0 mt-2 text-xs">实际执行位置：{{ downloaderLocation(selectedDownloader) }}。任务创建后不会自动换节点或回退主 Server。</p></div><DownloadRouteTargetPicker v-model="submitForm.mediaLibraryID" :preview="routePreview" :loading="routePreviewLoading" /><div v-if="selectedDownloader?.type === 'pan115_offline'" class="semantic-inset p-3 text-sm md:col-span-2"><span class="text-subtle block text-xs">115 下载目录</span><strong>{{ selectedDownloader.storage_name }} · {{ selectedDownloader.provider_directory_path || '/' }}</strong><span class="text-subtle mt-1 block text-xs">{{ sourceMode === 'share' ? '分享内容会转存到独立 omc-* 任务子目录，完成后自动识别整理。' : '离线任务会写入独立 omc-* 任务子目录，完成后按媒体库规则自动整理。' }}</span></div><div><label class="label">任务名称（可选）</label><input v-model="submitForm.displayName" class="input" maxlength="256" placeholder="留空时使用安全的通用名称" /></div><div><label class="label">队列优先级</label><input v-model.number="submitForm.priority" class="input" type="number" min="-100" max="100" /></div><div v-if="sourceMode === 'url' || sourceMode === 'share'" class="md:col-span-2"><label class="label">{{ sourceMode === 'share' ? '115 分享链接（每行一个，需包含提取码）' : '磁力链接或 HTTP(S) URL（每行一个）' }}</label><textarea v-model="submitForm.sourceURL" class="input min-h-24 font-mono text-xs" required autocomplete="off" spellcheck="false" :placeholder="sourceMode === 'share' ? '每行一个：\nhttps://115.com/s/...?password=...' : '每行一个，最多 50 个：\nmagnet:?xt=...\nhttps://...'" /><p class="text-subtle mb-0 mt-2 text-xs">空行会忽略，完全相同的链接只提交一次；部分失败时仅保留失败行。</p><p v-if="unsupportedNodePan115Source" class="semantic-warning mb-0 mt-2 p-3 text-xs">传输节点首版只支持 115 磁力离线和 115 分享；当前内容包含普通 HTTP(S) 离线链接，请改用运行在主 Server 的 115 下载器。</p></div><div v-else class="md:col-span-2"><label class="label">种子文件</label><input ref="fileInput" class="input" type="file" accept=".torrent,application/x-bittorrent" required @change="selectTorrent" /></div></div>
      <div v-if="selectedTarget" class="semantic-inset mt-4 grid gap-3 p-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div><span class="text-subtle block text-xs">入库方式</span><strong>{{ selectedRoute?.route_label }} · {{ selectedTarget.transfer_mode }}</strong></div>
      ...
      <button class="btn-primary mt-5" :disabled="saving || routePreviewLoading || enabledDownloaders.length === 0 || !!unsupportedNodePan115Source || !selectedRoute?.enabled || !selectedTarget || ((selectedDownloader?.type !== 'pan115_offline' || selectedRoute?.requires_managed_staging) && downloadSettings?.configured === false)">确认并入队</button>
    </form>

    <section v-if="activeSection === 'active' || activeSection === 'history'" :id="`download-panel-${activeSection}`" class="mt-6" role="tabpanel" :aria-labelledby="`download-tab-${activeSection}`"><div class="flex flex-wrap items-end justify-between gap-3"><div><h2 class="m-0 text-xl">{{ activeSection === 'history' ? '下载历史' : '进行中的下载流水线' }}</h2><p class="text-subtle mb-0 mt-1 text-xs">{{ activeSection === 'history' ? '仅展示已取消或下载、整理和做种后续均已成功收口的记录。' : '下载失败或后续整理、做种仍未完成的任务也会留在这里，避免问题被历史记录掩盖。' }}</p></div><ClearHistoryButton v-if="activeSection === 'history' && auth.canAny([Permissions.JobsControlOwn, Permissions.JobsControlAll])" scope="downloads" @cleared="load(false, true)" /></div>
          ...
          <tbody>
            <template v-for="task in visibleTasks" :key="task.id">
              <tr>
                ...
                <td><span :class="downloadStatusClass(task, isTaskRetrying(task))">{{ downloadStatusLabel(task, isTaskRetrying(task)) }}</span><span v-if="task.wait_reason" class="text-subtle mt-1 block text-xs" role="status">{{ task.wait_reason.message }}</span></td>
                <td class="min-w-36"><strong v-if="task.target_library_id" class="block">{{ task.target_library_name }}</strong><span v-else class="text-subtle block">仅下载</span><span v-if="task.target_library_id" class="text-subtle mt-1 block text-xs">{{ transferRouteLabel(task.route_kind) }} · {{ transferModeLabel(task.transfer_mode) }} · {{ transferPhaseLabel(task.transfer_phase) }}</span><span v-if="task.transfer_phase === 'risk_backoff'" class="semantic-warning-text mt-1 block text-xs">仅真实 405/429 或“操作频繁”才进入风控退避</span></td>
                ...
                <td><div v-if="canReadTransfers && task.transfer_task_id || canControl(task) || canDelete(task)" class="flex flex-wrap gap-2"><RouterLink v-if="canReadTransfers && task.transfer_task_id" class="btn-secondary" :to="{ name: 'organization', query: { task: task.transfer_task_id, scope: task.lifecycle_scope === 'history' ? 'history' : 'active' } }">查看整理详情</RouterLink><button v-if="canReorganize(task)" class="btn-secondary" type="button" :disabled="saving" @click="openReorganization(task)">修正识别并重新整理</button><button v-if="canControl(task) && ['queued','running','retry_wait'].includes(task.job_status)" class="btn-secondary" type="button" :disabled="saving || isTaskRetrying(task)" @click="control(task, 'pause')">暂停</button><button v-if="canControl(task) && task.job_status === 'paused'" class="btn-secondary" type="button" :disabled="saving || isTaskRetrying(task)" @click="control(task, 'resume')">恢复</button><button v-if="canControl(task) && isCompletedRecognitionFailure(task)" class="btn-secondary" type="button" :disabled="saving || isTaskRetrying(task)" @click="retryRecognitionImport(task)">{{ isTaskRetrying(task) ? '正在重试…' : '重新识别并入库' }}</button><button v-else-if="canControl(task) && task.job_status === 'failed'" class="btn-secondary" type="button" :disabled="saving || isTaskRetrying(task)" @click="control(task, 'retry')">{{ isTaskRetrying(task) ? '正在重试…' : '重试下载' }}</button><button v-if="canControl(task) && isCompletedRecognitionFailure(task)" class="btn-secondary" type="button" :disabled="saving || isTaskRetrying(task)" @click="openRecognitionRecovery(task)">人工介入</button><button v-if="canControl(task) && task.transfer_job_status === 'failed'" class="btn-secondary" type="button" :disabled="saving" @click="openRetarget(task)">修改入库目标</button><button v-if="canControl(task) && task.transfer_job_status === 'failed'" class="btn-secondary" type="button" :disabled="saving" @click="retryStage(task.transfer_job_id, '入库任务')">重试入库</button><button v-if="canControl(task) && canCancelDownloadPipeline(task)" class="btn-danger" type="button" :disabled="saving || isTaskRetrying(task)" @click="cancelDownloadPipeline(task)">取消</button><button v-if="canDelete(task) && (['failed','cancelled'].includes(task.job_status) || task.lifecycle_scope === 'history')" class="btn-danger" type="button" :disabled="saving || isTaskRetrying(task)" @click="deleteDownload(task)">{{ ['completed','cancelled'].includes(task.job_status) ? '删除历史记录' : '删除' }}</button></div><span v-else class="text-subtle text-xs">只读</span></td>
              </tr>
    ...
    <section v-if="activeSection === 'seeding'" id="download-panel-seeding" class="mt-6" role="tabpanel" aria-labelledby="download-tab-seeding"><div class="flex flex-wrap items-end justify-between gap-3"><div><h2 class="m-0 text-xl">做种管理</h2><p class="text-subtle mb-0 mt-1 text-xs">复制和软链接入库任务在这里跟踪。复制清理源文件；软链接永远保留源文件。</p></div><ClearHistoryButton v-if="canClearSeedingHistory" scope="seeding" label="清除已结束记录" @cleared="load(false, true)" /></div>
    ...
    <section v-if="activeSection === 'downloaders'" id="download-panel-downloaders" class="mt-6" role="tabpanel" aria-labelledby="download-tab-downloaders"><div class="flex flex-wrap items-end justify-between gap-4"><div><h2 class="m-0 text-xl">下载器管理</h2><p class="text-subtle mb-0 mt-1 text-xs">BT 下载器使用系统暂存目录；115 下载器在这里绑定所属数据源和下载目录。</p></div><button v-if="auth.can(Permissions.DownloadersCreate)" class="btn-primary" @click="createOpen = !createOpen">{{ createOpen ? '取消添加' : '添加下载器' }}</button></div>
      <form v-if="createOpen" class="panel mt-5 grid gap-4 md:grid-cols-2" @submit.prevent="createDownloader"><div><label class="label">名称</label><input v-model="createForm.name" class="input" required maxlength="128" /></div><div><label class="label">下载器类型</label><select v-model="createForm.type" class="input" aria-describedby="downloader-type-help"><option v-for="option in providerOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><p id="downloader-type-help" class="text-subtle mb-0 mt-2 text-xs">115 使用网盘自带的离线下载能力，不是 BT 客户端，因此不会进入做种管理。</p></div><div v-if="createForm.type !== 'fake'" class="md:col-span-2"><label class="label">运行位置</label><select v-model="createForm.executionLocation" class="input"><option value="server">主 Server</option><option value="node">传输节点</option></select><p class="text-subtle mb-0 mt-2 text-xs">选择节点后，下载、跨盘上传和做种流量都留在节点；主 Server 只识别媒体、冻结方案和显示进度。</p></div><div v-if="createForm.type !== 'fake' && createForm.executionLocation === 'node'" class="md:col-span-2"><label class="label">传输节点</label><select v-model="createForm.nodeID" class="input" required><option value="" disabled>请选择能力完整的在线节点</option><option v-for="node in createTransferNodes" :key="node.id" :value="node.id">{{ node.name }} · {{ node.platform }} {{ node.architecture }}</option></select><p v-if="createTransferNodes.length === 0" class="semantic-warning mb-0 mt-2 p-3 text-xs">没有支持当前下载器的在线节点，请先完成 Node 安装、配对和测试。节点只安装 OhMyCine Node，不会自动安装 qBittorrent。</p></div><div v-if="createForm.type === 'qbittorrent'" class="md:col-span-2"><label class="label">qBittorrent Web UI 地址</label><input v-model="createForm.baseURL" class="input" required placeholder="http://127.0.0.1:WebUI端口" /><p class="text-subtle mb-0 mt-2 text-xs">{{ createForm.executionLocation === 'node' ? '填写节点能访问的地址，通常是节点本机或容器网络地址。' : '端口以 qBittorrent「选项 → Web UI」中显示的端口为准，不是 BT 监听端口。' }}</p></div><div v-if="createForm.type === 'qbittorrent'"><label class="label">Web UI 用户名</label><input v-model="createForm.username" class="input" autocomplete="username" /></div><div v-if="createForm.type === 'qbittorrent'"><label class="label">Web UI 密码</label><SecretInput v-model="createForm.password" class="input" autocomplete="new-password" /></div><template v-if="createForm.type === 'qbittorrent' && createForm.executionLocation === 'node'"><div><label class="label">qBittorrent 保存根目录</label><input v-model="createForm.downloaderSaveRoot" class="input font-mono" required placeholder="/downloads" /><p class="text-subtle mb-0 mt-2 text-xs">qBittorrent API 看到的路径。</p></div><div><label class="label">节点挂载根目录</label><input v-model="createForm.nodeMountRoot" class="input font-mono" required placeholder="/var/lib/ohmycine-node/managed/downloads" /><p class="text-subtle mb-0 mt-2 text-xs">同一目录在节点系统中的真实路径，必须位于节点受管根内。</p></div></template><div v-if="createForm.type === 'pan115_offline'" class="md:col-span-2"><div class="semantic-inset mb-4 p-3 text-sm"><strong>115 网盘原生离线下载</strong><span class="text-subtle mt-1 block text-xs">{{ createForm.executionLocation === 'node' ? '公网 Node 接收冻结任务，从来源 115 下载到受管暂存后再上传目标网盘或回传本地库。' : '任务由主 Server 连接的 115 云端完成；不支持做种。' }}</span></div><label class="label">115 数据源</label><select v-model.number="createForm.storageID" class="input" required @change="resetCreateDirectory"><option :value="0" disabled>请选择支持离线下载的 115 数据源</option><option v-for="storage in pan115Storages" :key="storage.id" :value="storage.id">{{ storage.name }} · {{ storage.root_display_path || '/' }}</option></select><label class="label mt-4">数据源内下载目录</label><div class="flex gap-2"><input class="input font-mono" :value="createForm.directoryPath" readonly placeholder="请浏览并选择目录" /><button class="btn-secondary" type="button" :disabled="!createForm.storageID" @click="openDownloaderPicker('create')">浏览</button></div><p v-if="pan115Storages.length === 0" class="semantic-warning mb-0 mt-3 p-3 text-xs">当前没有可用的 115 数据源目录。请先在“数据源”中创建并启用 115 数据源。</p><p v-else class="text-subtle mb-0 mt-2 text-xs">可选择数据源根目录或其任意子目录；目录身份由 Server 校验并保存。</p></div><label class="text-muted flex items-center gap-3 text-sm"><input v-model="createForm.enabled" type="checkbox" />创建后启用</label><button class="btn-primary md:col-span-2" :disabled="saving || (createForm.type === 'pan115_offline' && (!createForm.storageID || !createForm.directoryToken)) || (createForm.type !== 'fake' && createForm.executionLocation === 'node' && (!createForm.nodeID || (createForm.type === 'qbittorrent' && (!createForm.downloaderSaveRoot || !createForm.nodeMountRoot))))">保存下载器</button></form>
      <label v-if="createOpen && createForm.type === 'pan115_offline' && createForm.executionLocation === 'server'" class="semantic-inset mt-3 flex items-start gap-3 p-3 text-sm"><input v-model="createForm.autoListenLifeEvents" class="mt-1" type="checkbox" :disabled="!createLifeEventDefault" /><span><strong>自动监听生活事件</strong><small v-if="createLifeEventDefault" class="text-subtle mt-1 block">手工内容将入库到“{{ createLifeEventDefault.name }}”；OhMyCine 的 omc-* 任务目录会跳过。</small><small v-else class="semantic-warning-text mt-1 block">请先在媒体库管理中，为该 115 账号设置唯一默认入库库。<RouterLink class="semantic-link ml-1" to="/system/media-libraries">前往设置</RouterLink></small></span></label>
      ...
      <div v-if="editing && editing.type !== 'fake'" class="semantic-inset mt-3 grid gap-4 p-4 md:grid-cols-2">
        <div class="md:col-span-2"><span class="label">运行位置</span><strong class="mt-1 block">{{ downloaderLocation(editing) }}</strong><p class="text-subtle mb-0 mt-2 text-xs">下载器已有任务时，Server 会拒绝更换执行节点；已创建任务永远使用自己的冻结节点。</p></div>
        <div v-if="editForm.executionLocation === 'node'" class="md:col-span-2"><label class="label">传输节点</label><select v-model="editForm.nodeID" class="input"><option value="" disabled>请选择能力完整的在线节点</option><option v-for="node in editTransferNodes" :key="node.id" :value="node.id">{{ node.name }} · {{ node.platform }} {{ node.architecture }}</option></select></div>
        <div v-if="editForm.executionLocation === 'node' && editing.type === 'qbittorrent'"><label class="label">qBittorrent 保存根目录</label><input v-model="editForm.downloaderSaveRoot" class="input font-mono" placeholder="/downloads" /></div>
        <div v-if="editForm.executionLocation === 'node' && editing.type === 'qbittorrent'"><label class="label">节点挂载根目录</label><input v-model="editForm.nodeMountRoot" class="input font-mono" placeholder="/var/lib/ohmycine-node/managed/downloads" /></div>
      </div>
      <label v-if="editing?.type === 'pan115_offline' && editForm.executionLocation === 'server'" class="semantic-inset mt-3 flex items-start gap-3 p-3 text-sm"><input v-model="editForm.autoListenLifeEvents" class="mt-1" type="checkbox" :disabled="!editLifeEventDefault" /><span><strong>自动监听生活事件</strong><small v-if="editLifeEventDefault" class="text-subtle mt-1 block">监听当前下载目录，手工内容默认入库到“{{ editLifeEventDefault.name }}”。<RouterLink class="semantic-link ml-1" to="/system/media-libraries">修改默认库</RouterLink></small><small v-else class="semantic-warning-text mt-1 block">当前 115 账号没有默认入库库，不能开启监听。<RouterLink class="semantic-link ml-1" to="/system/media-libraries">前往设置</RouterLink></small></span></label>
    </section>
    <TransferNodesPanel v-if="activeSection === 'nodes'" class="mt-6" />
    ...
    <div v-if="deletingTask" class="modal-backdrop fixed inset-0 z-60 flex items-center justify-center p-4" @click.self="!saving && (deletingTask = null)">
      <form class="panel w-full max-w-lg" role="dialog" aria-modal="true" aria-labelledby="download-delete-title" @submit.prevent="confirmDeleteDownload">
        <h2 id="download-delete-title" class="m-0 text-xl">删除下载任务</h2>
        <p class="page-description mt-2">{{ deletingTask.display_name }}</p>
        <p class="semantic-warning mt-4 p-3 text-sm">默认会从下载器删除任务并保留已下载文件，然后清理 OhMyCine 执行记录。下载器暂时不可用时不会删除本地记录。</p>
        <label class="semantic-inset mt-4 flex items-start gap-3 p-3 text-sm">
          <input v-model="deleteSourceData" class="mt-1" type="checkbox" />
          <span><strong>完全删除下载任务及源/临时文件</strong><small class="text-subtle mt-1 block">高风险操作。勾选后 Server 会要求下载器同时删除数据；默认不勾选。</small></span>
        </label>
        <div class="mt-5 flex justify-end gap-3"><button class="btn-secondary" type="button" :disabled="saving" @click="deletingTask = null">取消</button><button class="btn-danger" :disabled="saving">{{ saving ? '正在删除…' : '确认删除' }}</button></div>
      </form>
    </div>
    ...
    <MediaReorganizationDialog v-if="reorganizingTask" :open="true" :transfer-task-id="reorganizingTask.transfer_task_id" :download-task-id="reorganizingTask.id" :display-name="reorganizingTask.display_name" :current-title="reorganizingTask.scrape_title" :current-media-type="reorganizingTask.scrape_media_type" @close="reorganizingTask = null" @queued="reorganizationQueued" />
  </section>
</template>
