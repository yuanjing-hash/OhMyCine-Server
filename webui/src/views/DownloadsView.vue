...
import { beginDownloadRetry, canCancelDownloadPipeline, compatibleDownloadLibraries, downloadErrorMessage, downloadProviderStatusLabel, downloadStatusClass, downloadStatusLabel, formatBytes, formatETA, formatProgress, formatSampleTime, isDownloadHistoryTask, reconcileDownloadRetries, summarizeDownloaderTasks, torrentToBase64, type DownloadManagementSection, type DownloadRetryPresentations, type DownloadSourceMode } from '@/downloads'
...
const availableLibraries = computed(() => compatibleDownloadLibraries(mediaLibraries.value, storages.value, selectedDownloader.value))
...
<template>
  <section class="mx-auto max-w-7xl">
    ...
    <section v-if="activeSection === 'downloaders'" id="download-panel-downloaders" class="mt-6" role="tabpanel" aria-labelledby="download-tab-downloaders"><div class="flex flex-wrap items-end justify-between gap-4"><div><h2 class="m-0 text-xl">下载器管理</h2><p class="text-subtle mb-0 mt-1 text-xs">BT 下载器使用系统暂存目录；115 下载器在这里绑定所属数据源和下载目录。</p></div><button v-if="auth.can(Permissions.DownloadersCreate)" class="btn-primary" @click="createOpen = !createOpen">{{ createOpen ? '取消添加' : '添加下载器' }}</button></div>
      ...
      <label v-if="createOpen && createForm.type === 'pan115_offline'" class="semantic-inset mt-3 flex items-start gap-3 p-3 text-sm"><input v-model="createForm.autoListenLifeEvents" class="mt-1" type="checkbox" /><span><strong>自动监听生活事件</strong><small class="text-subtle mt-1 block">自动接管下载目录内由 115 App 手工加入的普通内容；OhMyCine 的 omc-* 任务目录会跳过。</small></span></label>
      ...
      <label v-if="editing?.type === 'pan115_offline'" class="semantic-inset mt-3 flex items-start gap-3 p-3 text-sm"><input v-model="editForm.autoListenLifeEvents" class="mt-1" type="checkbox" /><span><strong>自动监听生活事件</strong><small class="text-subtle mt-1 block">监听当前下载目录并接管 115 App 手工内容；omc-* 由下载 Worker 独占。</small></span></label>
    </section>
