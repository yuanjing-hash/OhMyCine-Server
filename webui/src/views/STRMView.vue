...
async function reconcile(mode: 'incremental'|'full') { saving.value=true; try { await reconcileSTRM(selectedID.value,mode); notify(mode==='full'?'完整核对已入队':'增量刷新已入队','success'); await load(true) } catch(reason){notify(message(reason),'error')} finally{saving.value=false} }
  ...
  <section class="page-shell">
    ...
    <div v-else class="grid gap-5 xl:grid-cols-[20rem_minmax(0,1fr)]">
      ...
      <main v-if="selected" class="min-w-0">
        <section class="panel"><div class="flex flex-wrap items-start justify-between gap-4"><div><h2 class="m-0">{{ selected.name }}</h2><p class="text-subtle mb-0 mt-2 text-sm">已应用 / 当前 generation：{{ selected.artifact_applied_generation }} / {{ selected.artifact_generation }} · 最近更新 {{ dateTime(selected.artifact_updated_at) }}</p></div><div class="flex flex-wrap gap-2"><button v-if="auth.can(Permissions.StrmRunsCreate)" class="btn-primary" :disabled="saving" @click="reconcile('incremental')">立即增量</button><button v-if="auth.can(Permissions.StrmRunsCreate)" class="btn-secondary" :disabled="saving" @click="reconcile('full')">完整核对</button><button v-if="auth.can(Permissions.StrmCleanup)" class="btn-danger" :disabled="saving" @click="previewCleanup">清理预览</button></div></div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3"><div class="management-tabs mt-4" role="tablist"><button class="management-tab" :class="activeTab==='runs'?'management-tab--active':''" @click="activeTab='runs'">运行历史</button><button class="management-tab" :class="activeTab==='artifacts'?'management-tab--active':''" @click="activeTab='artifacts'">托管产物</button></div><ClearHistoryButton v-if="activeTab==='runs'&&auth.can(Permissions.StrmCleanup)" class="mt-4" scope="strm_runs" :resource-id="String(selectedID)" label="清除运行历史" @cleared="loadDetails" /></div>
