        ...
        <template v-else>
          <div class="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <template v-if="cookieCloudForm.mode !== 'disabled'">
              <div><label class="label" for="cookiecloud-uuid">用户 KEY / UUID</label><input id="cookiecloud-uuid" v-model="cookieCloudForm.uuid" class="input font-mono" :required="!cookieCloudSettings?.credential_configured" autocomplete="off" :placeholder="cookieCloudSettings?.credential_configured ? '留空保留原值' : ''" /></div>
              <div><label class="label" for="cookiecloud-password">端到端加密密码</label><input id="cookiecloud-password" v-model="cookieCloudForm.password" class="input" type="password" :required="!cookieCloudSettings?.credential_configured" autocomplete="new-password" :placeholder="cookieCloudSettings?.credential_configured ? '留空保留原值' : ''" /></div>
              <div v-if="cookieCloudForm.mode === 'local'" class="sm:col-span-2"><label class="label" for="cookiecloud-auth">上传共享认证</label><input id="cookiecloud-auth" v-model="cookieCloudForm.authHeader" class="input font-mono" type="password" minlength="12" :required="!cookieCloudSettings?.credential_configured" autocomplete="new-password" :placeholder="cookieCloudSettings?.credential_configured ? '留空保留原值' : '至少 12 个字符'" /><p class="text-subtle mb-0 mt-1 text-xs">浏览器扩展上传时必须携带相同的 <span class="font-mono">X-CookieCloud-Auth</span> 请求头。</p></div>
