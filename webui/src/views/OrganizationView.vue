    ...
    <div id="organization-records" class="panel mt-4 overflow-x-auto p-0" role="tabpanel" :aria-labelledby="`organization-tab-${scope}`">
      ...
      <table v-else class="semantic-table organization-table w-full">
        ...
        <tbody><tr v-for="item in items" :key="item.id" :class="{ 'task-action-required': item.job_status === 'waiting_user_action' }">
          ...
          <td><strong class="block">{{ item.library_name }}</strong><small class="text-subtle mt-1 block">{{ transferModeLabels[item.transfer_mode] }} · {{ conflictPolicyLabels[item.conflict_policy] }}</small></td>
          <td><span :class="transferStatusClass(item)">{{ transferStatusLabel(item) }}</span><small v-if="item.last_error_message" class="semantic-danger-text mt-1 block">{{ item.last_error_message }}</small></td>
